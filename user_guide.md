...
By default Kubeflow does not persist any work that is done within jupyter notebook. That means if container is destroyed or recreated, all it's contents, including users working notebooks etc. are going to be deleted. To enable persistence user will need to have default storage class for [persistent volumes](https://kubernetes.io/docs/concepts/storage/persistent-volumes/). If that's the case, persistency can be enabled by setting jupyterNotebookPVCMount.
