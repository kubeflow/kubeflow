...
def test_jupyter(env, namespace):
    ...
    conditions = ["Ready"]
    results = util.wait_for_cr_condition(
      api_client, GROUP, PLURAL, VERSION, namespace, name, conditions
    )
    ...
    url = (
      "https://{master}/api/v1/namespaces/{namespace}/services/{service}:80"
      "/proxy/"
    ).format(
