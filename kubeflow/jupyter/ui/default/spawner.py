...
class KubeFormSpawner(KubeSpawner):
    ...
    def options_from_form(self, formdata):
        ...
        if (
          not memory_readonly and 'memory' in formdata and formdata['memory'][0]
        ):
        ...
        if self._default_config_contains('workspaceVolume'):
            ...
            if (
              'type' in default_ws_volume
              and 'value' in default_ws_volume['type']
            ):
            ...
            if (
              'name' in default_ws_volume
              and 'value' in default_ws_volume['name']
            ):
            ...
            if (
              'size' in default_ws_volume
              and 'value' in default_ws_volume['size']
            ):
                ws_volume['size'] = \
            ...
            if (
              'mountPath' in default_ws_volume
              and 'value' in default_ws_volume['mountPath']
            ):
                ws_volume['mountPath'] = \
            ...
            if (
              'accessModes' in default_ws_volume
              and 'value' in default_ws_volume['accessModes']
            ):
        ...
        if not ws_volume_readonly:
            if (
              not ws_type_readonly and 'ws_type' in formdata
              and formdata['ws_type'][0]
            ):
                ws_volume['type'] = formdata['ws_type'][0].strip()

            if (
              not ws_name_readonly and 'ws_name' in formdata
              and formdata['ws_name'][0]
            ):
                ws_volume['name'] = formdata['ws_name'][0].strip()

            if (
              not ws_size_readonly and 'ws_size' in formdata
              and formdata['ws_size'][0]
            ):
                ws_volume['size'] = '%sGi' % formdata['ws_size'][0].strip()

            if (
              not ws_mount_path_readonly and 'ws_mount_path' in formdata
              and formdata['ws_mount_path'][0]
            ):
                ws_volume['mountPath'] = \
                    formdata['ws_mount_path'][0].strip()

            if (
              not ws_access_modes_readonly and 'ws_access_modes' in formdata
        ...
        if (
          not extra_resources_readonly and 'extraResources' in formdata
          and formdata['extraResources'][0]
        ):
            options['extraResources'] = \
    ...
    def _get_pvc_manifest(
      self, name, storage_class, access_modes, storage, labels, annotations
    ):
        """
    ...
    def _provision_new_pvc(self, volume, namespace):
        ...
        try:
            ...
            if e.status == 409:
                self.log.warning(
                  'PVC %s already exists. New PVC not created.', volume['name']
                )
