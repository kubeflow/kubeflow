...
func main() {
	...
	gcServer, err := app.NewGcServer(s.KfctlAppsNamespace)
	if err != nil {
		log.Errorf("Failed creating GC Server: %v", err)
		return
	}
	if err := gcServer.StartGC(); err != nil {
		log.Errorf("Failed starting GC Server: %v", err)
		return
	}
}
