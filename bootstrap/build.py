...
def main(unparsed_args=None):
    parser = argparse.ArgumentParser(
        description=("Build bootstrapper image with ksonnet registries "
                     "specified in config.")
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=("Relative path to bootstrapper config file specify which "
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help=("e2e test target registry, format is <registry name>:<registry "
              "path>")
    )
    parser.add_argument(
        "--target",
    ...
    # Make a local dir, clone repos into it and Docker build will copy the
    # whole folder.
    ...
    for reg in conf['registries']:
        if test_reg_name == reg["name"]:
            ...
            sync_cmd = "rsync -a %s %s %s" % \
                       (test_reg_path, os_tmp_dir,
                        " ".join(["--exclude=" + term for term in excludes]))
            print(sync_cmd)
            os.system(sync_cmd)
        else:
            reg_path = os.path.join(os_tmp_dir, reg["name"])
            print("Adding registry %s from %s %s" % (
                reg["name"], reg["repo"], reg["version"]))
            os.system("git clone --depth 1 --branch %s %s %s" % (
                reg["version"], reg["repo"], reg_path))
    ...
    print("docker build %s %s -t %s --build-arg registries=%s --target=%s %s" %
          (args.build_opts, bargs, args.image, REG_FOLDER, args.target,
           FILE_PATH)
          )
    os.system(
        "docker build %s %s -t %s --build-arg registries=%s --target=%s %s" %
        (args.build_opts, bargs, args.image, REG_FOLDER,
