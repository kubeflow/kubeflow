...
func (gcp *Gcp) updateDM(resources kftypes.ResourceEnum) error {
	...
	if _, networkStatErr := os.Stat(path.Join(gcp.Spec.AppDir, GCP_CONFIG, NETWORK_FILE)); err != nil {
	...
	if _, gcfsStatErr := os.Stat(path.Join(gcp.Spec.AppDir, GCP_CONFIG, GCFS_FILE)); err != nil {
