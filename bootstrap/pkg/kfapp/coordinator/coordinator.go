...
import (
	...
	"github.com/kubeflow/kubeflow/bootstrap/v2/pkg/kfapp/aws"
...
type Builder interface {
	LoadKfAppCfgFile(cfgFile string) (kftypesv2.KfApp, error)
}

type DefaultBuilder struct {
}

func (b *DefaultBuilder) LoadKfAppCfgFile(cfgFile string) (kftypesv2.KfApp, error) {
	return LoadKfAppCfgFile(cfgFile)
}

func getConfigFromCache(pathDir string, kfDef *kfdefsv2.KfDef) ([]byte, error) {
	configPath := filepath.Join(pathDir, kftypesv2.DefaultConfigDir)
	overlays := []string{}
	...
	compPath := strings.Split(kftypesv2.DefaultConfigDir, "/")[1]
...
// GetPlatform will return an implementation of kftypesv2.GetPlatform that matches the platform string
// It looks for statically compiled-in implementations, otherwise throws unrecognized error
func getPlatform(kfdef *kfdefsv2.KfDef) (kftypesv2.Platform, error) {
	switch kfdef.Spec.Platform {
	case string(kftypesv2.MINIKUBE):
		return minikube.Getplatform(kfdef), nil
	case string(kftypesv2.GCP):
		return gcp.GetPlatform(kfdef)
	case string(kftypesv2.EXISTING_ARRIKTO):
		return existing_arrikto.GetPlatform(kfdef)
	case string(kftypesv2.AWS):
...
func (coord *coordinator) getPackageManagers(kfdef *kfdefsv2.KfDef) *map[string]kftypesv2.KfApp {
	var packagemanagers = make(map[string]kftypesv2.KfApp)
...
// getPackageManager will return an implementation of kftypesv2.KfApp that matches the packagemanager string
// It looks for statically compiled-in implementations, otherwise it delegates to
// kftypesv2.LoadKfApp which will try and dynamically load a .so
//
func getPackageManager(kfdef *kfdefsv2.KfDef) (kftypesv2.KfApp, error) {
	switch kfdef.Spec.PackageManager {
	case kftypesv2.KUSTOMIZE:
		return kustomize.GetKfApp(kfdef), nil
	case kftypesv2.KSONNET:
...
func CreateKfDefFromOptions(options map[string]interface{}) (*kfdefsv2.KfDef, error) {
	...
	appName := options[string(kftypesv2.APPNAME)].(string)
	...
	configFile := options[string(kftypesv2.CONFIG)].(string)
	...
	if configFile != "" {
		...
		platform := options[string(kftypesv2.PLATFORM)].(string)
		packageManager := options[string(kftypesv2.PACKAGE_MANAGER)].(string)
		version := options[string(kftypesv2.VERSION)].(string)
		useBasicAuth := options[string(kftypesv2.USE_BASIC_AUTH)].(bool)
		useIstio := options[string(kftypesv2.USE_ISTIO)].(bool)
		namespace := options[string(kftypesv2.NAMESPACE)].(string)
		project := options[string(kftypesv2.PROJECT)].(string)
		cacheDir := ""
		if options[string(kftypesv2.REPO)].(string) != "" {
			cacheDir = options[string(kftypesv2.REPO)].(string)
			...
			cacheDir, cacheDirErr = kftypesv2.DownloadToCache(appDir, kftypesv2.KubeflowRepo, version)
		...
		kfDef.Spec.Repo = path.Join(cacheDir, kftypesv2.KubeflowRepo)
		kfDef.Spec.Project = options[string(kftypesv2.PROJECT)].(string)
		kfDef.Spec.SkipInitProject = options[string(kftypesv2.SKIP_INIT_GCP_PROJECT)].(bool)
		...
		repoUri := repoVersionToUri(kftypesv2.KubeflowRepo, version)
		kfDef.Spec.Repos = append(kfDef.Spec.Repos, kfdefsv2.Repo{
			Name: kftypesv2.KubeflowRepoName,
			Uri:  repoUri,
		})
	}
	...
	disableUsageReport := options[string(kftypesv2.DISABLE_USAGE_REPORT)].(bool)
...
func NewKfApp(options map[string]interface{}) (kftypesv2.KfApp, error) {
	...
	r, ok := kfDef.Status.ReposCache[kftypesv2.ManifestsRepoName]
...
func backfillKfDefFromInitOptions(kfdef *kfdefsv2.KfDef, options map[string]interface{}) error {
	if kfdef.Spec.Platform == "" {
		if options[string(kftypesv2.PLATFORM)] != nil && options[string(kftypesv2.PLATFORM)].(string) != "" {
			kfdef.Spec.Platform = options[string(kftypesv2.PLATFORM)].(string)
	...
	if kfdef.Spec.Platform == kftypesv2.GCP {
		if kfdef.Spec.Project == "" {
			if options[string(kftypesv2.PROJECT)] != nil && options[string(kftypesv2.PROJECT)].(string) != "" {
	...
	if options[string(kftypesv2.PACKAGE_MANAGER)] != nil && options[string(kftypesv2.PACKAGE_MANAGER)].(string) != "" {
		if kfdef.Spec.PackageManager == "" {
			kfdef.Spec.PackageManager = options[string(kftypesv2.PACKAGE_MANAGER)].(string)
	...
	// Backfill repos
	if strings.Contains(kfdef.Spec.PackageManager, kftypesv2.KUSTOMIZE) {
		...
		for _, r := range kfdef.Spec.Repos {
			if r.Name == kftypesv2.ManifestsRepoName {
				hasRepo = true
			}
		}

		if hasRepo {
			log.Warnf("Repo %v exists in app.yaml ignoring version provided by --package-manager", kftypesv2.ManifestsRepoName)
		} else {
		...
		kfdef.Spec.PackageManager = kftypesv2.KUSTOMIZE
	}
	...
	if options[string(kftypesv2.CONFIG)] == nil {
		if options[string(kftypesv2.USE_BASIC_AUTH)] != nil {
			kfdef.Spec.UseBasicAuth = options[string(kftypesv2.USE_BASIC_AUTH)].(bool)
		}
		if options[string(kftypesv2.SKIP_INIT_GCP_PROJECT)] != nil {
			kfdef.Spec.SkipInitProject = options[string(kftypesv2.SKIP_INIT_GCP_PROJECT)].(bool)
		}
		if options[string(kftypesv2.DELETE_STORAGE)] != nil && kfdef.Spec.Platform == kftypesv2.GCP {
			kfdef.Spec.DeleteStorage = options[string(kftypesv2.DELETE_STORAGE)].(bool)
		}
	}
...
func backfillKfDefFromGenerateOptions(kfdef *kfdefsv2.KfDef, options map[string]interface{}) error {
	if kfdef.Spec.Platform == kftypesv2.GCP {
		if options[string(kftypesv2.EMAIL)] != nil && options[string(kftypesv2.EMAIL)].(string) != "" {
			if kfdef.Spec.Email == "" {
				kfdef.Spec.Email = options[string(kftypesv2.EMAIL)].(string)
			} else {
		...
		if kfdef.Spec.IpName == "" {
			if options[string(kftypesv2.IPNAME)] != nil && options[string(kftypesv2.IPNAME)].(string) != "" {
				kfdef.Spec.IpName = options[string(kftypesv2.IPNAME)].(string)

			} else if kfdef.Spec.Platform == kftypesv2.GCP && kfdef.Name != "" {
		...
		if kfdef.Spec.Hostname == "" {
			...
			if options[string(kftypesv2.HOSTNAME)] != nil && options[string(kftypesv2.HOSTNAME)].(string) != "" {
				kfdef.Spec.Hostname = options[string(kftypesv2.HOSTNAME)].(string)
		...
		if kfdef.Spec.Zone == "" {
			if options[string(kftypesv2.ZONE)] != nil && options[string(kftypesv2.ZONE)].(string) != "" {
				kfdef.Spec.Zone = options[string(kftypesv2.ZONE)].(string)
			} else {
				kfdef.Spec.Zone = kftypesv2.DefaultZone
			}
	...
	if options[string(kftypesv2.MOUNT_LOCAL)] != nil {
		kfdef.Spec.MountLocal = options[string(kftypesv2.MOUNT_LOCAL)].(bool)
	}
...
func LoadKfApp(options map[string]interface{}) (kftypesv2.KfApp, error) {
	appDir, err := os.Getwd()
	...
	cfgfile := filepath.Join(appDir, kftypesv2.KfConfigFile)
...
func LoadKfAppCfgFile(cfgfile string) (kftypesv2.KfApp, error) {
	...
	c := &coordinator{
		Platforms:       make(map[string]kftypesv2.Platform),
		PackageManagers: make(map[string]kftypesv2.KfApp),
		KfDef:           kfdef,
	}
...
// eg Platforms[kftypesv2.GCP], Platforms[kftypes.MINIKUBE], PackageManagers["kustomize"]
// The data attributes in kfdefsv2.KfDef are used by different KfApp implementations
type coordinator struct {
	Platforms       map[string]kftypesv2.Platform
	PackageManagers map[string]kftypesv2.KfApp
	KfDef           *kfdefsv2.KfDef
}

type KfDefGetter interface {
	GetKfDef() *kfdefsv2.KfDef
	GetPlugin(name string) (kftypesv2.KfApp, bool)
}
...
func (kfapp *coordinator) GetPlugin(name string) (kftypesv2.KfApp, bool) {
...
func (kfapp *coordinator) Apply(resources kftypesv2.ResourceEnum) error {
	platform := func() error {
	...
	k8s := func() error {
		for packageManagerName, packageManager := range kfapp.PackageManagers {
			packageManagerErr := packageManager.Apply(kftypesv2.K8S)
	...
	gcpAddedConfig := func() error {
		if kfapp.KfDef.Spec.Email == "" || kfapp.KfDef.Spec.Platform != kftypesv2.GCP {
			return nil
		}
...
func (kfapp *coordinator) Delete(resources kftypesv2.ResourceEnum) error {
	platform := func() error {
		if kfapp.KfDef.Spec.Platform != "" {
			platform := kfapp.Platforms[kfapp.KfDef.Spec.Platform]
			if platform != nil {
	...
	k8s := func() error {
		for packageManagerName, packageManager := range kfapp.PackageManagers {
			packageManagerErr := packageManager.Delete(kftypesv2.K8S)
			if packageManagerErr != nil {
				return &kfapis.KfError{
					Code: int(kfapis.INTERNAL_ERROR),
...
func (kfapp *coordinator) Generate(resources kftypesv2.ResourceEnum) error {
	platform := func() error {
		if kfapp.KfDef.Spec.Platform != "" {
			platform := kfapp.Platforms[kfapp.KfDef.Spec.Platform]
			if platform != nil {
	...
	k8s := func() error {
		for packageManagerName, packageManager := range kfapp.PackageManagers {
			packageManagerErr := packageManager.Generate(kftypesv2.K8S)
			if packageManagerErr != nil {
				return &kfapis.KfError{
					Code: int(kfapis.INTERNAL_ERROR),
...
func (kfapp *coordinator) Init(resources kftypesv2.ResourceEnum) error {
	platform := func() error {
		if kfapp.KfDef.Spec.Platform != "" {
			platform := kfapp.Platforms[kfapp.KfDef.Spec.Platform]
			if platform != nil {
	...
	k8s := func() error {
		for packageManagerName, packageManager := range kfapp.PackageManagers {
			packageManagerErr := packageManager.Init(kftypesv2.K8S)
			if packageManagerErr != nil {
				return &kfapis.KfError{
					Code: int(kfapis.INTERNAL_ERROR),
...
func (kfapp *coordinator) Show(resources kftypesv2.ResourceEnum, options map[string]interface{}) error {
	switch resources {
	case kftypesv2.K8S:
		fallthrough
	case kftypesv2.PLATFORM:
	...
	case kftypesv2.ALL:
		if kfapp.KfDef.Spec.Platform != "" {
			platform := kfapp.Platforms[kfapp.KfDef.Spec.Platform]
			show, ok := platform.(kftypesv2.KfShow)
			if ok && show != nil {
		...
		for packageManagerName, packageManager := range kfapp.PackageManagers {
			show, ok := packageManager.(kftypesv2.KfShow)
			if ok && show != nil {
				showErr := show.Show(kftypesv2.K8S, options)
				if showErr != nil {
					return &kfapis.KfError{
						Code: int(kfapis.INTERNAL_ERROR),
