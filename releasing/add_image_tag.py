...
def main(unparsed_args=None):  # pylint: disable=too-many-locals
    ...
    parser.add_argument(
      ...
      type=str,
      help=(
        "Regex pattern e.g. .*tensorflow.*notebook.*:v20180619.* "
    ...
    for image in config["images"]:
        ...
        if existing_index and not new_index:
            logging.error(
              "Not moving tag for image %s because no images matched %s", name,
              args.pattern
            )
            existing_index = []
        for e in existing_index:
            ...
            logging.info(
              "Image %s removing tag from sha %s", name,
              image["versions"][e]["digest"]
            )

        if len(new_index) > 1:
            raise ValueError(
              "Image {0} had {1} images match {2}".format(
