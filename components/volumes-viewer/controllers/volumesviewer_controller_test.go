...
var _ = Describe("VolumesViewer controller", func() {
	...
	var (
		...
		volumesViewer *kubefloworgv1alpha1.VolumesViewer
	)
	...
	BeforeEach(func() {
		...
		volumesViewerName := "test-volumesviewer"

		lookupKey = types.NamespacedName{Name: volumesViewerName, Namespace: testingNamespace}
		prefixedLookupKey = types.NamespacedName{Name: resourcePrefix + volumesViewerName, Namespace: testingNamespace}
		ns := &corev1.Namespace{
		...
		volumesViewer = &kubefloworgv1alpha1.VolumesViewer{
			ObjectMeta: metav1.ObjectMeta{
				Name:      volumesViewerName,
				Namespace: testingNamespace,
			},
			Spec: kubefloworgv1alpha1.VolumesViewerSpec{
		...
		Expect(k8sClient.Create(ctx, volumesViewer)).Should(Succeed())

		// We'll need to retry getting this newly created viewer, given that creation may not immediately happen.
		createdVolumesViewer := &kubefloworgv1alpha1.VolumesViewer{}
	...
	Context("When VolumesViewer created", func() {
		...
		BeforeEach(func() {
			expectedOwnerReferences = metav1.OwnerReference{
				...
				Kind:               "VolumesViewer",
				Name:               volumesViewer.Name,
				UID:                volumesViewer.UID,
		...
		It("Should CRUD Deployment", func() {
			...
			Expect(deployment.Spec.Template.Spec.Containers[0].Image).Should(Equal(volumesViewer.Spec.PodTemplate.Containers[0].Image))
			Expect(deployment.ObjectMeta.Labels).Should(And(
				HaveKeyWithValue(nameLabelKey, volumesViewer.Name),
				HaveKeyWithValue(instanceLabelKey, resourcePrefix+volumesViewer.Name),
				HaveKeyWithValue(partOfLabelKey, partOfLabelValue)))

			By("Updating the VolumeViewer")
		...
		It("Should CRUD Services", func() {
			...
			By("Adding a Service to the VolumeViewer")
			targetPort := 1234
			Eventually(func() error {
				viewer := &kubefloworgv1alpha1.VolumesViewer{}
				err := k8sClient.Get(ctx, lookupKey, viewer)
			...
			By("Updating the Service in the VolumeViewer")
			newTargetPort := 5678
			Eventually(func() error {
				viewer := &kubefloworgv1alpha1.VolumesViewer{}
				err := k8sClient.Get(ctx, lookupKey, viewer)
		...
		It("Should CRUD VirtualService", func() {
			...
			expectedRewrite := fmt.Sprintf("%s/%s/%s/", basePrefix, testingNamespace, volumesViewer.Name)
			...
			expectedRewrite = fmt.Sprintf("%s/%s/%s/", newBasePrefix, testingNamespace, volumesViewer.Name)
			Expect(http0["match"].([]interface{})[0].(map[string]interface{})["uri"].(map[string]interface{})["prefix"]).Should(Equal(expectedRewrite))
		...
		It("Should update status", func() {
			By("By checking an initial VolumesViewer status")
		...
		It("Strategically Merges Changes", func() {
			By("By updating the VolumesViewer")
	...
	Context("When RWO Scheduling is used", func() {
		...
		BeforeEach(func() {
			...
			// Update the VolumesViewer to use RWO Scheduling for each test
			viewer := &kubefloworgv1alpha1.VolumesViewer{}
			Eventually(func() error {
				err := k8sClient.Get(ctx, lookupKey, viewer)
				if err != nil {
		...
		// creates a RWO PVC,
		// lets the VolumesViewer mount the PVC
		...
		It("Should update Status.RWOVolumes", func() {
			By("Creating and Adding multiple PVCs to the VolumesViewer")
