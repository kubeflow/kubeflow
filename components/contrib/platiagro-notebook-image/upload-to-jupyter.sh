...
curl -v -X PUT http://server.anonymous:80/notebook/anonymous/server/api/contents/experiments/$1/operators/$2/$3 -H 'Content-Type: application/json' -H 'X-XSRFToken: token' -H 'Cookie: _xsrf=token' --data "{\"type\":\"notebook\", \"content\":$content}"
