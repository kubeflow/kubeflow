...
def cmap_status(cmap):
    """
    Set the status of the ConfigMap
    """
    if cmap.metadata.deletion_timestamp is not None:
        return status.create_status(status.STATUS_PHASE.TERMINATING,
                                    "Deleting ConfigMap...")

    if cmap.status.phase == "Bound":
        return status.create_status(status.STATUS_PHASE.READY, "Bound")

    # The ConfigMap is in Pending state, we check the Events to find out why
    evs = api.v1_core.list_namespaced_event(
        namespace=cmap.metadata.namespace,
        field_selector=api.events_field_selector(
            "PersistentVolumeClaim", cmap.metadata.name
        ),
    ).items

    # If there are no events, then the ConfigMap was just created
    if len(evs) == 0:
