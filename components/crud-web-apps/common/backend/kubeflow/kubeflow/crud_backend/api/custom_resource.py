...
def create_custom_rsrc(group, version, kind, data, namespace, auth=True):
    if auth:
...
def list_custom_rsrc(group, version, kind, namespace, auth=True):
    if auth:
...
def get_custom_rsrc(group, version, kind, namespace, name, auth=True):
    if auth:
