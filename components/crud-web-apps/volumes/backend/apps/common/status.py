...
def pvc_status(pvc):
    ...
    if pvc.metadata.deletion_timestamp is not None:
        return status.create_status(
            status.STATUS_PHASE.TERMINATING, "Deleting Volume...", key={"key": "volume.backend.status.volumeDeleting", "params": None}
        )

    if pvc.status.phase == "Bound":
        return status.create_status(status.STATUS_PHASE.READY, "Bound", key={"key": "volume.backend.status.bound", "params": None})
    ...
    if len(evs) == 0:
        return status.create_status(
            status.STATUS_PHASE.WAITING, "Provisioning Volume...", key={"key": "volume.backend.status.volumeProvisioning", "params": None}
        )
    ...
    return status.create_status(phase, msg, state, key={"key": "volume.backend.status.unavailableStatus", "params": None})
