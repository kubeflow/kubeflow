...
func (gcp *GcpWorkloadIdentity) updateWorkloadIdentity(namespace string, ksa string, f func(*iam.Policy, string)) error {
	...
	// Update policy
	// Use ProjectID from the default credentials for identity namespace.
	bindingMember := fmt.Sprintf("serviceAccount:%v.svc.id.goog[%v/%v]", credentials.ProjectID, namespace, ksa)
