...
def run(
  command, cwd=None, env=None, polling_interval=datetime.timedelta(seconds=1)
):
    """Run a subprocess.
...
def get_config(context_dir, version):
    ...
    config_file = os.path.join(
      context_dir, "versions", version, "version-config.json"
    )
...
def build_tf_serving(args):
    ...
    image_name = "{}/tensorflow-serving-{}:{}".format(
      args.registry, version, args.tag
    )
...
def main():
    ...
    parser.add_argument(
      "--tf_version", default="1.6", help="Tensorflow version"
    )
