...
# ~/CIS-LBK/functions/recommendations/nix_fed_ensure_nftables_table_exists.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       06/30/22    Recommendation "Ensure an nftables table exists"
#

fed_ensure_nftables_table_exists()
{
	...
	fed_ensure_nftables_table_exists_chk()
	{
		echo "- Start check - Ensure an nftables table exists" | tee -a "$LOG" 2>> "$ELOG"
		output="" l_iptables_rules="" l_ip6tables_rules=""
      ...
      l_nftables=$(nft list tables)
		
		# If nftables table exists, we pass
		if [ -n "$l_nftables" ] ; then
			echo "- An nftables table exists:\n$l_nftables" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure an nftables table exists" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
         echo -e "- No nftables table exists" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure an nftables table exists" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	
	fed_ensure_nftables_table_exists_fix()
	{
		echo -e "- Start remediation - Ensure an nftables table exists" | tee -a "$LOG" 2>> "$ELOG"
		
      echo -e "- Run \"nft create table inet <table name>\" to create a table according to your local site policies." | tee -a "$LOG" 2>> "$ELOG"
      test=manual
