...
# ~/CIS-LBK/functions/recommendations/nix_fed_ensure_firewalld_default_zone_set.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       05/18/22    Recommendation "Ensure firewalld default zone is set"
#

fed_ensure_firewalld_default_zone_set()
{
	...
	ensure_firewalld_default_zone_set_chk()
	{
		echo "- Start check - Is the firewalld default zone set" | tee -a "$LOG" 2>> "$ELOG"
		l_output=""
		...
		if $G_PQ firewalld >/dev/null; then
			l_fwddz="$(firewall-cmd --get-default-zone)"
			[ -z "$l_fwddz" ] && l_output="- FirewallD default zone not set"
		else
			[ -z "$l_output" ] && l_output="- FirewallD package isn't installed"
		fi
			
		# If package doesn't exist, we pass
		if [ -z "$l_output" ]; then
			echo -e "- The firewalld default zone is: \"$l_fwddz\"" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Is the firewalld default zone set" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	ensure_firewalld_default_zone_set_fix()
	{
		echo -e "- Start remediation - Set firewalld default zone" | tee -a "$LOG" 2>> "$ELOG"
		echo -e "- Setting default zone to \"public\"" | tee -a "$LOG" 2>> "$ELOG"
		firewall-cmd --set-default-zone=public
		echo -e "- End remediation - Set firewalld default zone"  | tee -a "$LOG" 2>> "$ELOG"
	}
