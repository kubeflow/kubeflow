...
# ~/CIS-LBK/functions/recommendations/nix_fed_ensure_nftables_default_deny_firewall_policy.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/2/22    Recommendation "Ensure nftables default deny firewall policy"
#
...
fed_ensure_nftables_default_deny_firewall_policy()
{
	...
	{
		echo -e "- Start check - Ensure nftables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"
		output=""
      ...
      # if ruleset passes, we pass
		if [ "$l_ruleset" = passed ]; then
			echo -e "- All base chains enforce a default deny policy" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure nftables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
			echo -e "- Some base chains do NOT enforce a default deny policy" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure nftables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	...
	{
		echo -e "- Start remediation - Ensure nftables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"
		
      echo -e "- Update the base chains to enforce a default policy of drop." | tee -a "$LOG" 2>> "$ELOG"
      echo -e "- These example rules can be used if desired:\n nft chain inet filter input { policy drop \; }\n nft chain inet filter forward { policy drop \; }\n nft chain inet filter output { policy drop \; }" | tee -a "$LOG" 2>> "$ELOG"
      test=manual
      
		echo -e "- End remediation - Ensure nftables default deny firewall policy" | tee -a "$LOG" 2>> "$ELOG"
	}
