...
# ~/CIS-LBK/functions/recommendations/nix_ensure_nodev_set_dev_shm_partition.sh
#
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/25/22    Recommendation "Ensure nodev option set on /dev/shm partition"

ensure_nodev_set_dev_shm_partition()
{
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_nodev_set_dev_shm_partition_chk()
   {
      echo -e "- Start check - Ensure nodev option set on /dev/shm partition" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure nodev option set on /dev/shm partition" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_nodev_set_dev_shm_partition_fix()
   {
      echo -e "- Start remediation - Ensure nodev option set on /dev/shm partition" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ "$l_rem_systemd" != "remediated" ]; then
         if grep -Pq "^\h*[^#]+\h+$XCCDF_VALUE_REGEX\h+" /etc/fstab; then
            ...
            echo "tmpfs           /dev/shm   tmpfs   rw,nosuid,nodev,noexec,inode64    0       0" >> /etc/fstab
            grep -P "^\h*[^#]+\h+$XCCDF_VALUE_REGEX\h+" /etc/fstab | grep -Pq "$l_option" && l_rem_fstab=remediated && G_REBOOT_REQUIRED=yes
            echo -e "- Mounting $XCCDF_VALUE_REGEX with noexec,nodev,nosuid" | tee -a "$LOG" 2>> "$ELOG"
            mount -o nosuid,nodev,noexec $XCCDF_VALUE_REGEX
         fi
      fi
      ...
      echo -e "- End remediation - Ensure nodev option set on /dev/shm partition" | tee -a "$LOG" 2>> "$ELOG"
   }
