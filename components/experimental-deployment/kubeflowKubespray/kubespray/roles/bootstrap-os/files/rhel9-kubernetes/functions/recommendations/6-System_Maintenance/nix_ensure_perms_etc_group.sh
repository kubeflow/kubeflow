...
# ~/CIS-LBK/functions/recommendations/nix_ensure_perms_etc_group.sh
# 
...
ensure_perms_etc_group()
{
	...
	nix_ensure_perms_etc_group_chk()
	{
		...
		# If $l_permissions does not equal "failed" and $l_uid does not equal "failed", we pass
	...
	nix_ensure_perms_etc_group_fix()
	{
