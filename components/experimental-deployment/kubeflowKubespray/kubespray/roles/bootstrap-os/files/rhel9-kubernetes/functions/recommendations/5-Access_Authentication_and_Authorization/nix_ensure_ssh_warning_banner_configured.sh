   ...
   ensure_ssh_warning_banner_configured_chk()
	{
      ...
      if ! $G_PQ openssh-server >/dev/null ; then
         ...
         if sshd -T -C user=root -C host="$(hostname)" -C addr="$(grep $(hostname) /etc/hosts | awk '{print $1}')" | grep -Eiq 'Banner\s\S+' && ! sshd -T -C user=root -C host="$(hostname)" -C addr="$(grep "$(hostname)" /etc/hosts | awk '{print $1}')" | grep -Eiq 'Banner\s+none' ; then
            l_output="- Entry found in sshd -T -C output: $(sshd -T -C user=root -C host="$(hostname)" -C addr="$(grep $(hostname) /etc/hosts | awk '{print $1}')" | grep -Ei 'Banner\s\S+')" && l_sshd_cmd="passed"
         else
            l_output="- No entry found in sshd -T -C output" && l_sshd_cmd="failed"
         fi
         
         if grep -Piq '^\s*Banner\s+none' /etc/ssh/sshd_config; then
            l_output="$l_output\n- Incorrect entry found in sshd_config: $(grep -Pi '^\s*Banner\s+none' /etc/ssh/sshd_config)" && l_sshd_config="failed"
         elif grep -Piq '^\s*#\s*Banner\s+' /etc/ssh/sshd_config; then
            l_output="$l_output\n- Commented entry found in sshd_config: $(grep -Pi '^\s*#\s*Banner\s+' /etc/ssh/sshd_config)" && l_sshd_config="failed"
         elif grep -Piq '^\s*Banner\s\S+' /etc/ssh/sshd_config; then
            l_output="$l_output\n- Entry found in sshd_config: $(grep -Pi '^\s*Banner\s\S+' /etc/ssh/sshd_config)" && l_sshd_config="passed"
         else
   ...
   ensure_ssh_warning_banner_configured_fix()
	{
      ...
      if grep -Piq '^\h*(#\s*)?Banner\s+' /etc/ssh/sshd_config; then
