...
# ~/CIS-LBK/functions/recommendations/nix_ensure_actions_another_user_always_logged.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure actions as another user are always logged"
# David Neilson		 02/04/23	 Modified to use recent syscall script.
# David Neilson      05/30/23    Added word boundaries in lines containing "rule" variables

ensure_actions_another_user_always_logged()
{
	echo
	...
	fix_auditd_syscall_rule(){
        ...
        for rule in "${rules_found_array[@]}"; do
            ...
            if (( ${#missing_parameter_array[@]} > 0 )); then
                ...
                rule_line="$(grep -Pihn -- "^\h*$rule(*\h*|\h+.*)" /etc/audit/rules.d/*.rules 2> /dev/null | awk -F':' '{print $1}')"
                ...
                rule_line_regex="$(grep -Po -- '^\h*.+-S\h+([\S+,])?\S+\h*' <<< "$rule")"
    ...
    ensure_actions_another_user_always_logged_chk()
	{
		echo "- Start check - Ensure actions as another user are always logged" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""
        
        # Set the syscalls we're interested in
		l_rule_1="execve"
		l_parameters_rule_1="-C euid!=uid:-C (euid!=uid|uid!=euid);-F auid!=unset:-F auid!=(unset|-1|4294967295)"

        if [ "$l_sysarch" = "b64" ]; then
            # Verify b64 ondisk rules
            echo -e "- Checking arch=b64 ondisk rules for execve" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule "arch=b64" "ondisk" "$l_rule_1" "$l_parameters_rule_1"


            # Verify b64 auditctl entry
            echo -e "- Checking arch=b64 auditctl rules for execve" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule "arch=b64" "auditctl" "$l_rule_1" "$l_parameters_rule_1"
        fi

        # Verify b32 ondisk rules
        echo -e "- Checking arch=b32 ondisk rules for execve" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule "arch=b32" "ondisk" "$l_rule_1" "$l_parameters_rule_1"

        # Verify b32 auditctl entry
        echo -e "- Checking arch=b32 auditctl rules for execve" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule "arch=b32" "auditctl" "$l_rule_1" "$l_parameters_rule_1"

        if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure actions as another user are always logged" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_actions_another_user_always_logged_fix()
	{
		echo "- Start remediation - Ensure actions as another user are always logged" | tee -a "$LOG" 2>> "$ELOG"

        if [ "$l_sysarch" = "b64" ]; then
            # Fix b64 ondisk rules
            echo -e "- Fixing arch=b64 ondisk rules for execve" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule "arch=b64" "$l_rule_1" "$l_parameters_rule_1" "user_emulation" "/etc/audit/rules.d/50-user_emulation.rules"
        fi
	
        # Fix b32 ondisk rules
        echo -e "- Fixing arch=b32 ondisk rules for execve" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_syscall_rule "arch=b32" "$l_rule_1" "$l_parameters_rule_1" "user_emulation" "/etc/audit/rules.d/50-user_emulation.rules"

        echo "- End remediation - Ensure actions as another user are always logged" | tee -a "$LOG" 2>> "$ELOG"
    }
