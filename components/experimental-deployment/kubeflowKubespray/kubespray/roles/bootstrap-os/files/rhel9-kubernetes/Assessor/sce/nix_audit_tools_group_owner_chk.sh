...
# -------------------------------------------------------------------
# E. Pinnell   08/21/23   check audit tools files group owner
...
for l_fname in "${a_audit_tools[@]}"; do
   if [ -e "$l_fname" ]; then
      ...
      if [[ ! "$l_file_group_owner" =~ $l_group_owner\s*$ ]]; then
         l_output2="$l_output2\n - file: \"$l_fname\" is owned by: \"$l_file_group_owner\" (should be owned by: \"${l_group_owner/|/ or }\")"
      fi
   fi
done
...
if [ -z "$l_output2" ]; then
   if [ "$l_count" -gt "0" ]; then
      l_output=" - All audit tools files are owned by: \"${l_group_owner/|/ or }\""
   else
