...
# ~/CIS-LBK/functions/recommendations/nix_fed_ensure_perms_etc_shadow_dash.sh
# 
...
# David Neilson	     09/24/22	  Minor syntax changes
fed_ensure_perms_etc_shadow_dash()
{
	...
	fed_ensure_perms_etc_shadow_dash_chk()
	{
		...
		if [ "$l_permissions" != "failed" ] && [ "$l_ownergroup" != "failed" ]; then
			...
			[ "$l_perms" != "$l_perms_correct" ] && echo "- /etc/shadow- permissions are set to \"$l_perms\"" | tee -a "$LOG" 2>> "$ELOG"
		   	[ "$l_uid" != "$l_uid_correct" ] && [ "$l_gid" != "$l_gid_correct" ] && echo "- /etc/shadow- owner is \"$l_uid\" and group is \"$l_gid\"" | tee -a "$LOG" 2>> "$ELOG"
		   	echo "- End check - /etc/shadow- permissions and ownership" | tee -a "$LOG" 2>> "$ELOG"
		   	return "${XCCDF_RESULT_FAIL:-102}"
		fi	
	}

	fed_ensure_perms_etc_shadow_dash_fix()
	{
