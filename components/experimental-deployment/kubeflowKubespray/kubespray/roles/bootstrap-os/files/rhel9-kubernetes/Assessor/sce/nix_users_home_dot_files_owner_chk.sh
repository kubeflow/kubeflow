...
# -------------------------------------------------------------------
# E. Pinnell   04/19/21   Check that users home directory dot files belong to the user or root
...
awk -F: '($1!~/(halt|sync|shutdown|nfsnobody)/ && $7!~/^(\/usr)?\/sbin\/nologin(\/)?$/ && $7!~/(\/usr)?\/bin\/false(\/)?$/) {print $1 " " $6}' /etc/passwd | (while read -r user dir; do
	if [ ! -d "$dir" ]; then
		...
		for file in $(find "$dir"/ -maxdepth 1 -xdev -type f -name '.*'); do
			if [ -e "$file" ]; then
				fown=$(stat -L -c "%U" "$file")
				if [ "$fown" != "$user" ] && [ "$fown" != "root" ]; then
					[ -z "$output" ] && output=" User: \"$user\" File: \"$file\" is owned by \"$fown\"" || output="$output,\n User: \"$user\" File: \"$file\" is owned by \"$fown\""
				fi
			fi
		done
	fi
...
if [ "$passing" = true ] ; then
	echo "Passed.  All files in users home directorys belong to the user or root"
