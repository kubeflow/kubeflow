...
ensure_bogus_icmp_responses_ignored()
{
	...
	{
        echo "- Start check - Ensure bogus ICMP responses are ignored" | tee -a "$LOG" 2>> "$ELOG" 
        l_output="" l_output2="" 
        ...
        if [ -z "$l_output2" ]; then 
            echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
            echo "- End check - Ensure bogus ICMP responses are ignored" | tee -a "$LOG" 2>> "$ELOG"
	...
	{
        echo "- Start remediation - Ensure bogus ICMP responses are ignored" | tee -a "$LOG" 2>> "$ELOG"
        l_output="" l_output2="" 
        ...
        echo "- End remediation - Ensure bogus ICMP responses are ignored" | tee -a "$LOG" 2>> "$ELOG"
	}
