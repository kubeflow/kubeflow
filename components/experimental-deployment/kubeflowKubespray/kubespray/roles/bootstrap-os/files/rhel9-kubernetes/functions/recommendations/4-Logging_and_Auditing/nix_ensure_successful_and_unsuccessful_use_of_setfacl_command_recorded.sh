...
# ~/CIS-LBK/functions/recommendations/nix_ensure_successful_and_unsuccessful_use_of_setfacl_command_recorded.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure successful and unsuccessful attempts to use the setfacl command are recorded"
# David Neilson	     04/06/23	 Modified to work on both fed- and deb-.

ensure_successful_and_unsuccessful_use_of_setfacl_command_recorded()
{
	echo
	...
	ensure_successful_and_unsuccessful_use_of_setfacl_command_recorded_chk()
	{
		echo "- Start check - Ensure successful and unsuccessful attempts to use the setfacl command are recorded" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

        # Set the commands we're interested in
		l_rule_1="/usr/bin/setfacl"
        ...
        echo -e "- Checking ondisk rules for /usr/bin/setfacl" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_command_rule "noarch" "ondisk" "$l_rule_1" "$l_parameters_rule_1"

        # Verify auditctl entry with command
        echo -e "- Checking auditctl rules for /usr/bin/setfacl" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_command_rule "noarch" "auditctl" "$l_rule_1" "$l_parameters_rule_1"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure successful and unsuccessful attempts to use the setfacl command are recorded" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_successful_and_unsuccessful_use_of_setfacl_command_recorded_fix()
	{
		echo "- Start remediation - Ensure successful and unsuccessful attempts to use the setfacl command are recorded" | tee -a "$LOG" 2>> "$ELOG"
		
        # Fix ondisk rules with command
        echo -e "- Fixing ondisk rules for /usr/bin/setfacl" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_command_rule "noarch" "$l_rule_1" "$l_parameters_rule_1" "perm_chng" "/etc/audit/rules.d/50-perm_chng.rules"

		echo "- End remediation - Ensure successful and unsuccessful attempts to use the setfacl command are recorded" | tee -a "$LOG" 2>> "$ELOG"	
	}
