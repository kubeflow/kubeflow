...
# ~/CIS-LBK/functions/recommendations/nix_ensure_web_server_not_installed.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       11/23/22    Recommendation "Ensure a web server is not installed"
# 

ensure_web_server_not_installed()
{
	...
	ensure_web_server_not_installed_chk()
	{
		l_output=""
		l_pkgmgr=""

		echo -e "- Start check - Ensure a web server is not installed" | tee -a "$LOG" 2>> "$ELOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ httpd | grep "not installed" && $G_PQ nginx | grep "not installed"; then
						...
						echo -e "- End check - Ensure a web server is not installed" | tee -a "$LOG" 2>> "$ELOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
				...
				;;
				*dpkg*)
					if $G_PQ apache2; then	
						# If avahi-daemon is not installed, this command returns a "1", which means we go to the else clause and the test passes
						echo -e "- FAILED:\n- apache2 package installed on the system" | tee -a "$LOG" 2>> "$ELOG"
						echo -e "- End check - Ensure a web server is not installed" | tee -a "$LOG" 2>> "$ELOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
						echo -e "- PASSED:\n- apache2 package not installed" | tee -a "$LOG" 2>> "$ELOG"
						echo -e "- End check - Ensure a web server is not installed" | tee -a "$LOG" 2>> "$ELOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	ensure_web_server_not_installed_fix()
	{
		echo -e "- Start remediation - Ensure a web server is not installed" | tee -a "$LOG" 2>> "$ELOG"
		...
		echo -e "- End remediation - Ensure a web server is not installed" | tee -a "$LOG" 2>> "$ELOG"
	}
