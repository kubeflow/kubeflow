...
# David Neilson      03/28/23    Made fed- only to reference /etc/sysconfig/network* 
# David Neilson		 05/29/23	 Resolved issue with not working with auditctl.
...
fed_ensure_events_modify_systems_network_environment_collected()
{
	...
	l_test=""
	l_uid_min=""
	l_locales="issue issue.net hosts sysconfig/network sysconfig/network-scripts"
	...
	fix_auditd_syscall_rule_filtered(){
        ...
        for rule in "${rules_found_array[@]}"; do
            ...
            if (( ${#missing_parameter_array[@]} > 0 )); then
                ...
                rule_line="$(grep -Pihn -- "$rule(*\h*|\h+.*)" /etc/audit/rules.d/*.rules | awk -F':' '{print $1}')"
	...
	{
		echo "- Start check - Ensure events that modify the systems network environment are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2="" 
		...
		# Verify "-w /etc/$l_locales -p wa -k <VALUE>" ondisk rule exists
		for l_locale in $l_locales; do
			echo -e "- Checking \"-w /etc/$l_locale\" ondisk rules for sethostname,setdomainname" | tee -a "$LOG" 2>> "$ELOG"
			if grep -Pqs -- "^\h*-w\h+\/etc\/$l_locale\h+-p\h+wa\h+-k\h+\S+\b" /etc/audit/rules.d/*.rules || grep -Pqs -- "^\h*-w\h+\/etc\/$l_locale\h+-p\h+wa\h+-F\h+key=\S+\b" /etc/audit/rules.d/*.rules; then
				l_output="$l_output\n- ondisk rule:\"-w /etc/$l_locale -p wa -k system-locale\" correctly configured"
			else 
		    	l_output2="$l_output2\n- No ondisk rule for:  \"-w /etc/$l_locale -p wa -k system-locale\""
       		fi
		done

		# Verify "-w /etc/$l_locale -p wa -k <VALUE>" auditctl rule exists
		for l_locale in $l_locales; do
			echo -e "- Checking \"-w /etc/$l_locale\" auditctl rules for sethostname,setdomainname" | tee -a "$LOG" 2>> "$ELOG"
			if auditctl -l | grep -Pqs -- "^\h*-w\h+\/etc\/$l_locale\h+-p\h+wa\h+-k\h+\S+\b" || auditctl -l | grep -Pqs -- "^\h*-w\h+\/etc\/$l_locale\h+-p\h+wa\h+-F\h+key=\S+\b" ; then
				l_output="$l_output\n- auditctl rule:\"-w /etc/$l_locale -p wa -k system-locale\" correctly configured"
			else 
				l_output2="$l_output2\n- No auditctl rule for:  \"-w /etc/$l_locale -p wa -k system-locale\""
			fi
		done

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure events that modify the systems network environment are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	{
		echo "- Start remediation - Ensure events that modify the systems network environment are collected" | tee -a "$LOG" 2>> "$ELOG"

		# Fix -w /etc rules
		for l_locale in $l_locales; do
			if ! grep -Pqs -- "^\h*-w\h+\/etc\/$l_locale\h+-p\h+wa\h+-k\h+\S+\b" /etc/audit/rules.d/*.rules && ! grep -Pqs -- "^\h*-w\h+\/etc\/$l_locale\h+-p\h+wa\h+-F\h+key=\S+\b" /etc/audit/rules.d/*.rules; then
				echo -e "- Fixing \"-w /etc/$l_locale\" ondisk rule for sethostname,setdomainname" | tee -a "$LOG" 2>> "$ELOG"
				echo "-w /etc/$l_locale -p wa -k system-locale" >> /etc/audit/rules.d/50-system-locale.rules
			fi 
		done
       	...
       	if [ "$l_sysarch" = "b64" ]; then
            ...
            fix_auditd_syscall_rule_filtered "arch=b64" "$l_rule_1" "$l_parameters_rule_1" "system-locale" "/etc/audit/rules.d/50-system-locale.rules"
        fi
	
	    # Fix b32 ondisk rules
        echo -e "- Fixing arch=b32 ondisk rules for sethostname,setdomainname" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_syscall_rule_filtered "arch=b32" "$l_rule_1" "$l_parameters_rule_1" "system-locale" "/etc/audit/rules.d/50-system-locale.rules"
		        
		echo "- End remediation - Ensure events that modify the systems network environment are collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
