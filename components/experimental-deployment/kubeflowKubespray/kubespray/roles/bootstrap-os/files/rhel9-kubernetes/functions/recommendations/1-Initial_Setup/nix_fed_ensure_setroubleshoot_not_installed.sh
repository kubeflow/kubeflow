...
fed_ensure_setroubleshoot_not_installed()
{
	...
	fed_ensure_setroubleshoot_not_installed_chk()
	{
		l_output=""
		l_pkgmgr=""
		...
		if [ -z "$l_output" ]; then
			if $G_PQ setroubleshoot | grep "not installed" > /dev/null; then
				echo -e "- PASSED:\n- setroubleshoot package not found" | tee -a "$LOG" 2>> "$ELOG"
				echo -e "- End check - setroubleshoot" | tee -a "$LOG" 2>> "$ELOG"
				return "${XCCDF_RESULT_PASS:-101}"
			else
				echo -e "- FAILED:\n- setroubleshoot package installed on the system" | tee -a "$LOG" 2>> "$ELOG"
				echo -e "- End check - setroubleshoot package" | tee -a "$LOG" 2>> "$ELOG"
	...
	fed_ensure_setroubleshoot_not_installed_fix()
	{
		echo -e "- Start Remediation - removing setroubleshoot package" | tee -a "$LOG" 2>> "$ELOG"
		$G_PR setroubleshoot
		echo -e "- End Remediation - setroubleshoot" | tee -a "$LOG" 2>> "$ELOG"
	}	
