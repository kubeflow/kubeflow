...
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure dac permissions modification events collected"
# David Neilson      02/16/23    Updated to latest Ubuntu benchmark.
...
ensure_dac_permission_modification_events_collected()
{
	...
	fix_auditd_syscall_rule_filtered(){
        ...
        for rule in "${rules_found_array[@]}"; do
            ...
            if (( ${#missing_parameter_array[@]} > 0 )); then
                ...
                rule_line="$(grep -Pihn -- "$rule\b" /etc/audit/rules.d/*.rules | awk -F':' '{print $1}')"
	...
	ensure_dac_permission_modification_events_collected_chk()
	{
		echo "- Start check - Ensure dac permissions modification events collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

		l_rule_1="chmod,fchmod,fchmodat"
		l_parameters_rule_1="-F auid>=1000:-F auid>=1000;-F auid!=unset:-F auid!=(unset|-1|4294967295)"
		l_rule_2="chown,fchown,lchown,fchownat"
  		l_parameters_rule_2="-F auid>=1000:-F auid>=1000;-F auid!=unset:-F auid!=(unset|-1|4294967295)"
        l_rule_3="setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr"
 		l_parameters_rule_3="-F auid>=1000:-F auid>=1000;-F auid!=unset:-F auid!=(unset|-1|4294967295)"


        if [ "$l_sysarch" = "b64" ]; then
            # Verify b64 ondisk rules
            echo -e "- Checking arch=b64 ondisk rules for chmod,fchmod,fchmodat" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "ondisk" "$l_rule_1" "$l_parameters_rule_1"
            echo -e "- Checking arch=b64 ondisk rules for chown,fchown,fchownat,lchown" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "ondisk" "$l_rule_2" "$l_parameters_rule_2"
            echo -e "- Checking arch=b64 ondisk rules for setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "ondisk" "$l_rule_3" "$l_parameters_rule_3"

            # Verify b64 auditctl entry
            echo -e "- Checking arch=b64 auditctl rules for chmod,fchmod,fchmodat" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "auditctl" "$l_rule_1" "$l_parameters_rule_1"
            echo -e "- Checking arch=b64 auditctl rules for chown,fchown,fchownat,lchown" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "auditctl" "$l_rule_2" "$l_parameters_rule_2"
            echo -e "- Checking arch=b64 auditctl rules for setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "auditctl" "$l_rule_3" "$l_parameters_rule_3"
        fi

        # Verify b32 ondisk rules
        echo -e "- Checking arch=b32 ondisk rules for chmod,fchmod,fchmodat" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "ondisk" "$l_rule_1" "$l_parameters_rule_1"
        echo -e "- Checking arch=b32 ondisk rules for chown,fchown,fchownat,lchown" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "ondisk" "$l_rule_2" "$l_parameters_rule_2"
        echo -e "- Checking arch=b32 ondisk rules for setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "ondisk" "$l_rule_3" "$l_parameters_rule_3"

        # Verify b32 auditctl entry
        echo -e "- Checking arch=b32 auditctl rules for chmod,fchmod,fchmodat" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "auditctl" "$l_rule_1" "$l_parameters_rule_1"
        echo -e "- Checking arch=b32 auditctl rules for chown,fchown,fchownat,lchown" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "auditctl" "$l_rule_2" "$l_parameters_rule_2"
        echo -e "- Checking arch=b32 auditctl rules for setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "auditctl" "$l_rule_3" "$l_parameters_rule_3"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure dac permissions modification events collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_dac_permission_modification_events_collected_fix()
	{
		echo "- Start remediation - Ensure dac permissions modification events collected" | tee -a "$LOG" 2>> "$ELOG"

       if [ "$l_sysarch" = "b64" ]; then
            # Fix b64 ondisk rules
            echo -e "- Fixing arch=b64 ondisk rules for chmod,fchmod,fchmodat" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule_filtered "arch=b64" "$l_rule_1" "$l_parameters_rule_1" "perm_mods" "/etc/audit/rules.d/50-perm_mod.rules"
            echo -e "- Fixing arch=b64 ondisk rules for chown,fchown,fchownat,lchown" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule_filtered "arch=b64" "$l_rule_2" "$l_parameters_rule_2" "perm_mods" "/etc/audit/rules.d/50-perm_mod.rules"
            echo -e "- Fixing arch=b64 ondisk rules for setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule_filtered "arch=b64" "$l_rule_3" "$l_parameters_rule_3" "perm_mods" "/etc/audit/rules.d/50-perm_mod.rules"
        fi
	
	    # Fix b32 ondisk rules
            echo -e "- Fixing arch=b32 ondisk rules for chmod,fchmod,fchmodat" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule_filtered "arch=b32" "$l_rule_1" "$l_parameters_rule_1" "perm_mods" "/etc/audit/rules.d/50-perm_mod.rules"
            echo -e "- Fixing arch=b32 ondisk rules for chown,fchown,fchownat,lchown" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule_filtered "arch=b32" "$l_rule_2" "$l_parameters_rule_2" "perm_mods" "/etc/audit/rules.d/50-perm_mod.rules"
            echo -e "- Fixing arch=b32 ondisk rules for setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule_filtered "arch=b32" "$l_rule_3" "$l_parameters_rule_3" "perm_mods" "/etc/audit/rules.d/50-perm_mod.rules"

		echo "- End remediation - Ensure dac permissions modification events collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
