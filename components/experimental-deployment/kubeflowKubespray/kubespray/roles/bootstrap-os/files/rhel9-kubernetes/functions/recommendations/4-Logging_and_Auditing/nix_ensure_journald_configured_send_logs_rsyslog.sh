...
ensure_journald_configured_send_logs_rsyslog()
{
   ...
   ensure_journald_configured_send_logs_rsyslog_chk()
   {
		...
		if grep -Eq '^\s*[Ff]orward[Tt]o[Ss]yslog\s*=\s*yes\b' /etc/systemd/journald.conf; then
			echo -e "- PASS:\n- /etc/systemd/journald.conf contains: $(grep -E '^\s*[Ff]orward[Tt]o[Ss]yslog' /etc/systemd/journald.conf)" | tee -a "$LOG" 2>> "$ELOG"
			...
			if grep -Eq '^\s*(#)?\s*[Ff]orward[Tt]o[Ss]yslog' /etc/systemd/journald.conf; then
				echo -e "- FAIL:\n- /etc/systemd/journald.conf contains: $(grep -E '^\s*(#)?\s*[Ff]orward[Tt]o[Ss]yslog' /etc/systemd/journald.conf)" | tee -a "$LOG" 2>> "$ELOG"
				echo -e "- End check - Ensure journald is configured to send logs to rsyslog" | tee -a "$LOG" 2>> "$ELOG"
