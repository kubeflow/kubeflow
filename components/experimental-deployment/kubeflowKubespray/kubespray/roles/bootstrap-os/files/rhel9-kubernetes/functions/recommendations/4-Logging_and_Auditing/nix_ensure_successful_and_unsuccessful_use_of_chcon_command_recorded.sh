...
# ~/CIS-LBK/functions/recommendations/nix_ensure_successful_and_unsuccessful_use_of_chcon_command_recorded.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure successful and unsuccessful attempts to use the chcon command are recorded"
# David Neilson	     03/30/23	 Modified to work on both fed- and deb-.

ensure_successful_and_unsuccessful_use_of_chcon_command_recorded()
{
	echo
	...
	fix_auditd_command_rule(){
        ...
        for rule in "${rules_found_array[@]}"; do
            ...
            if (( ${#missing_parameter_array[@]} > 0 )); then
                parameter_insert_list=""
				###rule_file="$(grep -Pil -- "$rule" /etc/audit/rules.d/*.rules)" - This will not work if there is a comment at the beginning of the line
				rule_file="$(grep -Pil -- "^\h*$rule" /etc/audit/rules.d/*.rules)"
                ###rule_line="$(grep -Pihn -- "$rule" /etc/audit/rules.d/*.rules | awk -F':' '{print $1}')" - This will not work if there is a comment at the beginning of the line
				...
				###rule_line_regex="$(grep -Po -- '^.+-F\h+path=\S+\h*' <<< "$rule")" - Need to check for a specific string in front of the "-F path="
				rule_line_regex="$(grep -Po -- '^\h*-a\h+\S+\h+-F\h+path=\S+\h*' <<< "$rule")"

                echo -e "-   Adding $parameter_insert_list" | tee -a "$LOG" 2>> "$ELOG"
                ###sed -ri "$rule_line s|($rule_line_regex)(.*)$|\1$parameter_insert_list \2|" "$rule_file" - need a space in front of $parameter_insert_list
	...
	ensure_successful_and_unsuccessful_use_of_chcon_command_recorded_chk()
	{
		echo "- Start check - Ensure successful and unsuccessful attempts to use the chcon command are recorded" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

        # Set the commands we're interested in
		l_rule_1="/usr/bin/chcon"
        ...
        echo -e "- Checking ondisk rules for /usr/bin/chcon" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_command_rule "noarch" "ondisk" "$l_rule_1" "$l_parameters_rule_1"

        # Verify auditctl entry with command
        echo -e "- Checking auditctl rules for /usr/bin/chcon" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_command_rule "noarch" "auditctl" "$l_rule_1" "$l_parameters_rule_1"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure successful and unsuccessful attempts to use the chcon command are recorded" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_successful_and_unsuccessful_use_of_chcon_command_recorded_fix()
	{
		echo "- Start remediation - Ensure successful and unsuccessful attempts to use the chcon command are recorded" | tee -a "$LOG" 2>> "$ELOG"
		
        # Fix ondisk rules with command
        echo -e "- Fixing ondisk rules for /usr/bin/chcon" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_command_rule "noarch" "$l_rule_1" "$l_parameters_rule_1" "perm_chng" "/etc/audit/rules.d/50-perm_chng.rules"

		echo "- End remediation - Ensure successful and unsuccessful attempts to use the chcon command are recorded" | tee -a "$LOG" 2>> "$ELOG"	
	}
