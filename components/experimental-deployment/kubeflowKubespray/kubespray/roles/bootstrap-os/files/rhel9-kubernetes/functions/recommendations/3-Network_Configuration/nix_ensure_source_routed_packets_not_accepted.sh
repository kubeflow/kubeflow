...
ensure_source_routed_packets_not_accepted()
{
	...
	ensure_source_routed_packets_not_accepted_chk()
	{
		echo "- Start check - Ensure source routed packets are not accepted" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2="" 
		...
		if [ -z "$l_output2" ]; then 
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure source routed packets are not accepted" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_source_routed_packets_not_accepted_fix()
	{
		echo "- Start remediation - Ensure source routed packets are not accepted" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2="" 
		...
		echo "- End remediation - Ensure source routed packets are not accepted" | tee -a "$LOG" 2>> "$ELOG"
	}
