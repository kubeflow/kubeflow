...
# ~/CIS-LBK/functions/recommendations/nix_ensure_packet_redirect_sending_disabled.sh
# 
...
# Eric Pinnell       04/07/22    Modified to leverage updated search path
# Justin Brown		 11/20/22	 Refactored to use common functions
#

ensure_packet_redirect_sending_disabled()
{
	# Start recommendation entriey for verbose log and output to screen
	echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
	l_test=""
	
	ensure_packet_redirect_sending_disabled_chk()
	{
        echo "- Start check - Ensure packet redirect sending is disabled" | tee -a "$LOG" 2>> "$ELOG" 
        l_output="" l_output2="" 
        ...
        if [ -z "$l_output2" ]; then 
            echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
            echo "- End check - Ensure packet redirect sending is disabled" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_packet_redirect_sending_disabled_fix()
	{
        echo "- Start remediation - Ensure packet redirect sending is disabled" | tee -a "$LOG" 2>> "$ELOG"
        l_output="" l_output2="" 
        ...
        echo "- End remediation - Ensure packet redirect sending is disabled" | tee -a "$LOG" 2>> "$ELOG"
	}
