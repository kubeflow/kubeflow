...
ensure_ipv6_router_advertisements_not_accepted()
{
	...
	ensure_ipv6_router_advertisements_not_accepted_chk()
	{
		echo "- Start check - Ensure IPv6 router advertisements are not accepted" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2="" 
		...
		if [ -z "$l_output2" ]; then 
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure IPv6 router advertisements are not accepted" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_ipv6_router_advertisements_not_accepted_fix()
	{
		echo "- Start remediation - Ensure IPv6 router advertisements are not accepted" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2="" 
		...
		echo "- End remediation - Ensure IPv6 router advertisements are not accepted" | tee -a "$LOG" 2>> "$ELOG"
	}
