...
# ~/CIS-LBK/functions/recommendations/nix_ensure_nosuid_set_var_tmp_partition.sh
#
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/27/22    Recommendation "Ensure nosuid option set on /var/tmp partition"

ensure_nosuid_set_var_tmp_partition()
{

echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   test=""
   XCCDF_VALUE_REGEX="/var/tmp"
   l_option="nosuid"

   ensure_nosuid_set_var_tmp_partition_chk()
   {
   ...
   ensure_nosuid_set_var_tmp_partition_fix()
   {
      ...
      if [ "$l_rem_systemd" != "remediated" ]; then
         if grep -Pq "^\h*[^#]+\h+$XCCDF_VALUE_REGEX\h+" /etc/fstab; then
            ...
            echo -e "- For new installations, during installation create a custom partition setup and specify a separate partition for /var/tmp.\n- For systems that were previously installed, create a new partition and configure /etc/fstab as appropriate." | tee -a "$LOG" 2>> "$ELOG"
