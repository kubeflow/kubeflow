...
ensure_broadcast_icmp_requests_ignored()
{
	...
	ensure_broadcast_icmp_requests_ignored_chk()
	{
        echo "- Start check - Ensure broadcast ICMP requests are ignored" | tee -a "$LOG" 2>> "$ELOG" 
        l_output="" l_output2="" 
        ...
        if [ -z "$l_output2" ]; then 
            echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
            echo "- End check - Ensure broadcast ICMP requests are ignored" | tee -a "$LOG" 2>> "$ELOG"
	...
	ensure_broadcast_icmp_requests_ignored_fix()
	{
        echo "- Start remediation - Ensure broadcast ICMP requests are ignored" | tee -a "$LOG" 2>> "$ELOG"
        l_output="" l_output2="" 
        ...
        echo "- End remediation - Ensure broadcast ICMP requests are ignored" | tee -a "$LOG" 2>> "$ELOG"
	}
