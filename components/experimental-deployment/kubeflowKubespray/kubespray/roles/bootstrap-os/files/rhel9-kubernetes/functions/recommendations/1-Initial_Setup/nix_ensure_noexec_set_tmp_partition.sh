...
# ~/CIS-LBK/functions/recommendations/nix_ensure_noexec_set_tmp_partition.sh
#
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/25/22    Recommendation "Ensure noexec option set on /tmp partition"

ensure_noexec_set_tmp_partition()
{
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_noexec_set_tmp_partition_chk()
   {
      echo -e "- Start check - Ensure noexec option set on /tmp partition" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure noexec option set on /tmp partition" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_noexec_set_tmp_partition_fix()
   {
      echo -e "- Start remediation - Ensure noexec option set on /tmp partition" | tee -a "$LOG" 2>> "$ELOG"
      ...
      echo -e "- End remediation - Ensure noexec option set on /tmp partition" | tee -a "$LOG" 2>> "$ELOG"
   }
