...
fed_ensure_nftables_installed()
{
	...
	fed_ensure_nftables_installed_chk()
	{
		...
		! $G_PQ nftables >/dev/null && output="- The nftables package is not installed on the system"
		
		# If package doesn't exist, we pass
		if [ -z "$output" ] ; then
			echo "- The nftables package is installed" | tee -a "$LOG" 2>> "$ELOG"
			echo "- End check - Ensure nftables is installed" | tee -a "$LOG" 2>> "$ELOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	fed_ensure_nftables_installed_fix()
	{
