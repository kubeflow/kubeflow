...
# ~/CIS-LBK/functions/recommendations/nix_fed_ensure_at_restricted_authorized_users.sh
# 
...
fed_ensure_at_restricted_authorized_users()
{
	...
	fed_ensure_at_restricted_authorized_users_chk()
	{
		l_output=""
		l_fperm=""
		l_fog=""
	...
	fed_ensure_at_restricted_authorized_users_fix()
	{
		...
		if [ -f /etc/at.allow ]; then
			if ! stat -Lc "%a" /etc/at.allow | grep -Pq -- '^\h*[0,2,4,6]00\h*$'; then
