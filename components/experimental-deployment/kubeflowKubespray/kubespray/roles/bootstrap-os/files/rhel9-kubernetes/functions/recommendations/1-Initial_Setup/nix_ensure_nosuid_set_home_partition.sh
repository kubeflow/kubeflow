...
# ~/CIS-LBK/functions/recommendations/nix_ensure_nosuid_set_home_partition.sh
#
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/27/22    Recommendation "Ensure nosuid option set on /home partition"

ensure_nosuid_set_home_partition()
{

echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   test=""
   XCCDF_VALUE_REGEX="/home"
   l_option="nosuid"

   ensure_nosuid_set_home_partition_chk()
   {
      echo -e "- Start check - Ensure nosuid option set on /home partition" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure nosuid option set on /home partition" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_nosuid_set_home_partition_fix()
   {
      echo -e "- Start remediation - Ensure nosuid option set on /home partition" | tee -a "$LOG" 2>> "$ELOG"
            ...
            for l_dname in $l_unit_files; do
               ...
               if [ -n "$l_mfile" ] && systemctl list-unit-files --type mount "$l_mfile" &>/dev/null; then
                  if ! awk '/[Mount]/,0' $l_dname | grep -Eq "^\s*Options=([^#]+,)?$l_option"; then
                     ...
                     mount -o remount,nodev,nosuid $XCCDF_VALUE_REGEX
                  fi
               fi
            done
         fi
      ...
      if [ "$l_rem_systemd" != "remediated" ]; then
         if grep -Pq "^\h*[^#]+\h+$XCCDF_VALUE_REGEX\h+" /etc/fstab; then
            ...
            echo -e "- For new installations, during installation create a custom partition setup and specify a separate partition for /home.\n- For systems that were previously installed, create a new partition and configure /etc/fstab as appropriate." | tee -a "$LOG" 2>> "$ELOG"
      ...
      echo -e "- End remediation - Ensure nosuid option set on /home partition" | tee -a "$LOG" 2>> "$ELOG"
   }
