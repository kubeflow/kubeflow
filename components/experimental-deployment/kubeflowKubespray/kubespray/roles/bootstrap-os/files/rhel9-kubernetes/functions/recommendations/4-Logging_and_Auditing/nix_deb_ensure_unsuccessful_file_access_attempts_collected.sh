...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_unsuccessful_file_access_attempts_collected.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# J. Brown	         01/26/23    Recommendation "Ensure successful file system mounts are collected"
# 

deb_ensure_unsuccessful_file_access_attempts_collected()
{
	echo
	...
	find_ondisk_rule_arch_filtered(){
		if [ -z "$2" ]; then
			...
			grep -Pih -- "-a\h+(always,exit|exit,always)\h.*-F\h$1" /etc/audit/rules.d/*.rules | grep -Pv -- "$2"
		fi
	}
	...
	find_ondisk_rule_no_arch_filtered(){
		if [ -z "$1" ]; then
			...
			grep -Pih -- "-a\h+(always,exit|exit,always)\h" /etc/audit/rules.d/*.rules | grep -Pv -- "-F arch=" | grep -Pv -- "$1"
		fi
	}
	...
	find_auditctl_rule_arch_filtered(){
		if [ -z "$2" ]; then
			...
			auditctl -l | grep -Pi -- "-a\h+(always,exit|exit,always)\h.*-F\h$1" | grep -Pv -- "$2"
		fi
	}
	...
	find_auditctl_rule_no_arch_filtered(){
		if [ -z "$1" ]; then
			...
			auditctl -l | grep -Pi -- "-a\h+(always,exit|exit,always)\h" | grep -Pv -- "-F arch=" | grep -Pv -- "$1"
		fi
	}
	...
	deb_ensure_unsuccessful_file_access_attempts_collected_chk()
	{
		echo "- Start check - Ensure unsuccessful file access attempts are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

		# check the rule(s) for creat,open,openat,truncate,ftruncate -F exit=-EACCES

		# Set the syscalls we're interested in (creat,open,openat,truncate,ftruncate)
		l_rule_1="creat,open,openat,truncate,ftruncate"
		l_parameters_rule_1="-F exit=-EACCES:-F exit=-EACCES;-F auid>=$l_uid_min:-F auid>=$l_uid_min;-F auid!=unset:-F auid!=(unset|-1|auid!=4294967295)"

        if [ "$l_sysarch" = "b64" ]; then
            # Verify b64 rules with creat,open,openat,truncate,ftruncate
            echo -e "- Checking arch=b64 ondisk rules for creat,open,openat,truncate,ftruncate -F exit=-EACCES" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "ondisk" "$l_rule_1" "$l_parameters_rule_1" "-F exit=-EPERM"


            # Verify auditctl entry with creat,open,openat,truncate,ftruncate
            echo -e "- Checking arch=b64 auditctl rules for creat,open,openat,truncate,ftruncate -F exit=-EACCES" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "auditctl" "$l_rule_1" "$l_parameters_rule_1" "-F exit=-EPERM"
        fi

        # Verify b32 rules with creat,open,openat,truncate,ftruncate
        echo -e "- Checking arch=b32 ondisk rules for creat,open,openat,truncate,ftruncate -F exit=-EACCES" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "ondisk" "$l_rule_1" "$l_parameters_rule_1" "-F exit=-EPERM"


        # Verify auditctl entry with creat,open,openat,truncate,ftruncate
        echo -e "- Checking arch=b32 auditctl rules for creat,open,openat,truncate,ftruncate -F exit=-EACCES" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "auditctl" "$l_rule_1" "$l_parameters_rule_1" "-F exit=-EPERM"


        # check the rule(s) for creat,open,openat,truncate,ftruncate -F exit=-EPERM

        # Set the syscalls we're interested in (creat,open,openat,truncate,ftruncate)
		l_rule_2="creat,open,openat,truncate,ftruncate"
		l_parameters_rule_2="-F exit=-EPERM:-F exit=-EPERM;-F auid>=$l_uid_min:-F auid>=$l_uid_min;-F auid!=unset:-F auid!=(unset|-1|auid!=4294967295)"

        if [ "$l_sysarch" = "b64" ]; then
            # Verify b64 rules with creat,open,openat,truncate,ftruncate
            echo -e "- Checking arch=b64 ondisk rules for creat,open,openat,truncate,ftruncate -F exit=-EPERM" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "ondisk" "$l_rule_2" "$l_parameters_rule_2" "-F exit=-EACCES"


            # Verify auditctl entry with creat,open,openat,truncate,ftruncate
            echo -e "- Checking arch=b64 auditctl rules for creat,open,openat,truncate,ftruncate -F exit=-EPERM" | tee -a "$LOG" 2>> "$ELOG"
            eval_auditd_syscall_rule_filtered "arch=b64" "auditctl" "$l_rule_2" "$l_parameters_rule_2" "-F exit=-EACCES"
        fi

        # Verify b32 rules with creat,open,openat,truncate,ftruncate
        echo -e "- Checking arch=b32 ondisk rules for creat,open,openat,truncate,ftruncate -F exit=-EPERM" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "ondisk" "$l_rule_2" "$l_parameters_rule_2" "-F exit=-EACCES"


        # Verify auditctl entry with creat,open,openat,truncate,ftruncate
        echo -e "- Checking arch=b32 auditctl rules for creat,open,openat,truncate,ftruncate -F exit=-EPERM" | tee -a "$LOG" 2>> "$ELOG"
        eval_auditd_syscall_rule_filtered "arch=b32" "auditctl" "$l_rule_2" "$l_parameters_rule_2" "-F exit=-EACCES"

		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure unsuccessful file access attempts are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	deb_ensure_unsuccessful_file_access_attempts_collected_fix()
	{
		echo "- Start remediation - Ensure unsuccessful file access attempts are collected" | tee -a "$LOG" 2>> "$ELOG"

        # Fix rules for creat,open,openat,truncate,ftruncate -F exit=-EACCES
		
        if [ "$l_sysarch" = "b64" ]; then
            # Fix b64 rules with syscall creat,open,openat,truncate,ftruncate
            echo -e "- Fixing arch=b64 ondisk rules for creat,open,openat,truncate,ftruncate -F exit=-EACCES" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule_filtered "arch=b64" "$l_rule_1" "$l_parameters_rule_1" "perm_mod" "/etc/audit/rules.d/50-perm_mod.rules" "-F exit=-EPERM"
        fi

		# Fix b32 rules with syscall creat,open,openat,truncate,ftruncate
        echo -e "- Fixing arch=b32 ondisk rules for creat,open,openat,truncate,ftruncate -F exit=-EACCES" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_syscall_rule_filtered "arch=b32" "$l_rule_1" "$l_parameters_rule_1" "perm_mod" "/etc/audit/rules.d/50-perm_mod.rules" "-F exit=-EPERM"

        # Fix rules for creat,open,openat,truncate,ftruncate -F exit=-EPERM

        if [ "$l_sysarch" = "b64" ]; then
            # Fix b64 rules with syscall creat,open,openat,truncate,ftruncate
            echo -e "- Fixing arch=b64 ondisk rules for creat,open,openat,truncate,ftruncate -F exit=-EPERM" | tee -a "$LOG" 2>> "$ELOG"
            fix_auditd_syscall_rule_filtered "arch=b64" "$l_rule_2" "$l_parameters_rule_2" "perm_mod" "/etc/audit/rules.d/50-perm_mod.rules" "-F exit=-EACCES"
        fi

		# Fix b32 rules with syscall creat,open,openat,truncate,ftruncate
        echo -e "- Fixing arch=b32 ondisk rules for creat,open,openat,truncate,ftruncate -F exit=-EPERM" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_syscall_rule_filtered "arch=b32" "$l_rule_2" "$l_parameters_rule_2" "perm_mod" "/etc/audit/rules.d/50-perm_mod.rules" "-F exit=-EACCES"

		echo "- End remediation - Ensure unsuccessful file access attempts are collected" | tee -a "$LOG" 2>> "$ELOG"	
	}
