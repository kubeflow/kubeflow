   ...
   ensure_ssh_ignorerhosts_enabled_chk()
	{
      ...
      if ! $G_PQ openssh-server >/dev/null ; then
         ...
         if sshd -T -C user=root -C host="$(hostname)" -C addr="$(grep $(hostname) /etc/hosts | awk '{print $1}')" | grep -Eiq 'IgnoreRhosts\s+yes'; then
            l_output="- Entry found in sshd -T -C output: $(sshd -T -C user=root -C host="$(hostname)" -C addr="$(grep $(hostname) /etc/hosts | awk '{print $1}')" | grep -Ei 'IgnoreRhosts\s+yes')" && l_sshd_cmd="passed"
         else
            l_output="- No entry found in sshd -T -C output" && l_sshd_cmd="failed"
         fi
         
         if grep -Piq '^\s*IgnoreRhosts\s+no' /etc/ssh/sshd_config; then
            l_output="$l_output\n- Incorrect entry found in sshd_config: $(grep -Pi '^\s*IgnoreRhosts\s+no' /etc/ssh/sshd_config)" && l_sshd_config="failed"
         elif grep -Piq '^\s*#\s*IgnoreRhosts\s+' /etc/ssh/sshd_config; then
            l_output="$l_output\n- Incorrect entry found in sshd_config: $(grep -Pi '^\s*#\s*IgnoreRhosts\s+' /etc/ssh/sshd_config)" && l_sshd_config="failed"
         elif grep -Piq 'IgnoreRhosts\s+yes' /etc/ssh/sshd_config; then
            l_output="$l_output\n- Entry found in sshd_config: $(grep -Pi '^\s*IgnoreRhosts\s+yes' /etc/ssh/sshd_config)" && l_sshd_config="passed"
         else
   ...
   ensure_ssh_ignorerhosts_enabled_fix()
	{
      ...
      if grep -Piq '^\h*(#\s*)?IgnoreRhosts\s+' /etc/ssh/sshd_config; then
         ...
         sed -E -i '/^\s*\#\s*Authentication/a IgnoreRhosts no' /etc/ssh/sshd_config
      fi
