...
# ~/CIS-LBK/functions/recommendations/nix_ensure_var_log_audit_separate_partition.sh
#
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/25/22    Recommendation "Ensure separate partition exists for /var/log/audit"

ensure_var_log_audit_separate_partition()
{

echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   test=""

   ensure_var_log_audit_separate_partition_chk()
   {
      echo -e "- Start check - Ensure separate partition exists for /var/log/audit" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ "$l_partition_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure separate partition exists for /var/log/audit" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_var_log_audit_separate_partition_fix()
   {
      echo -e "- Start remediation - Ensure separate partition exists for /var/log/audit" | tee -a "$LOG" 2>> "$ELOG"
      ...
      echo -e "- End remediation - Ensure separate partition exists for /var/log/audit" | tee -a "$LOG" 2>> "$ELOG"
   }
