...
# ~/CIS-LBK/functions/recommendations/nix_ensure_nosuid_set_var_log_audit_partition.sh
#
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/27/22    Recommendation "Ensure nosuid option set on /var/log/audit partition"

ensure_nosuid_set_var_log_audit_partition()
{
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_nosuid_set_var_log_audit_partition_chk()
   {
      echo -e "- Start check - Ensure nosuid option set on /var/log/audit partition" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure nosuid option set on /var/log/audit partition" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_nosuid_set_var_log_audit_partition_fix()
   {
      echo -e "- Start remediation - Ensure nosuid option set on /var/log/audit partition" | tee -a "$LOG" 2>> "$ELOG"
      ...
      echo -e "- End remediation - Ensure nosuid option set on /var/log/audit partition" | tee -a "$LOG" 2>> "$ELOG"
   }
