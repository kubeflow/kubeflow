...
# ~/CIS-LBK/functions/recommendations/nix_ensure_nosuid_set_var_partition.sh
#
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Justin Brown       07/27/22    Recommendation "Ensure nosuid option set on /var partition"

ensure_nosuid_set_var_partition()
{
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_nosuid_set_var_partition_chk()
   {
      echo -e "- Start check - Ensure nosuid option set on /var partition" | tee -a "$LOG" 2>> "$ELOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure nosuid option set on /var partition" | tee -a "$LOG" 2>> "$ELOG"
   ...
   ensure_nosuid_set_var_partition_fix()
   {
      echo -e "- Start remediation - Ensure nosuid option set on /var partition" | tee -a "$LOG" 2>> "$ELOG"
            ...
            for l_dname in $l_unit_files; do
               ...
               if [ -n "$l_mfile" ] && systemctl list-unit-files --type mount "$l_mfile" &>/dev/null; then
                  if ! awk '/[Mount]/,0' $l_dname | grep -Eq "^\s*Options=([^#]+,)?$l_option"; then
                     ...
                     echo -e "- Remounting $XCCDF_VALUE_REGEX with nodev,nosuid" | tee -a "$LOG" 2>> "$ELOG"
      ...
      echo -e "- End remediation - Ensure nosuid option set on /var partition" | tee -a "$LOG" 2>> "$ELOG"
   }
