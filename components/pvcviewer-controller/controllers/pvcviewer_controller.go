...
// Reconcile reads that state of the cluster for a PVCViewer object and makes changes based on the state read
// and what is in the PVCViewer.Spec
// +kubebuilder:rbac:groups=pvcviewer.kubeflow.org,resources=pvcviewers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pvcviewer.kubeflow.org,resources=pvcviewers/status,verbs=get;update;patch
...
func (r *PVCViewerReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	ctx := context.Background()
	logger := r.Log.WithValues("pvcviewer", req.NamespacedName)
	...
	if _err != nil {
		...
		if len(foundDeployment.Status.Conditions) > 0 {
			condition := pvcviewerv1alpha1.PVCViewerCondition{
...
func generateDeployment(viewer *pvcviewerv1alpha1.PVCViewer, log logr.Logger, r *PVCViewerReconciler) (*appsv1.Deployment, error) {
	...
	if err, sch := rwoPVCScheduling(); err != nil {
		...
		//for the case that the PVCViewer Server is using a RWO PVC (as a log storage)
		//and the PVC is already mounted by another pod.

		//Get the PVC that will be accessed by the PVCViewer Server.
					...
					Containers: []corev1.Container{
						{
							...
							Env: []corev1.EnvVar{
								...
								corev1.EnvVar{
									Name:  "FB_DATABASE",
									Value: "/filebrowser/filebrowser.db",
								},
								corev1.EnvVar{
									Name:  "FB_BASEURL",
									Value: fmt.Sprintf("/pvcviewer/%s/%s/", viewer.Namespace, viewer.Name),
								},
								corev1.EnvVar{
									Name:  "FB_NOAUTH",
									Value: "true",
...
func generateService(viewer *pvcviewerv1alpha1.PVCViewer) *corev1.Service {
	return &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			Type:     "ClusterIP",
			Selector: map[string]string{"app": viewer.Name},
			Ports: []corev1.ServicePort{
				corev1.ServicePort{
					Name:       "http-" + viewer.Name,
					Port:       80,
...
func generateVirtualService(viewer *pvcviewerv1alpha1.PVCViewer) (*unstructured.Unstructured, error) {
	prefix := fmt.Sprintf("/pvcviewer/%s/%s/", viewer.Namespace, viewer.Name)
	rewrite := fmt.Sprintf("/pvcviewer/%s/%s/", viewer.Namespace, viewer.Name)
	service := fmt.Sprintf("%s.%s.svc.cluster.local", viewer.Name, viewer.Namespace)

	vsvc := &unstructured.Unstructured{}
	vsvc.SetAPIVersion("networking.istio.io/v1alpha3")
	vsvc.SetKind("VirtualService")
	vsvc.SetName(viewer.Name)
	vsvc.SetNamespace(viewer.Namespace)
...
func generateNodeAffinity(affinity *corev1.Affinity, pvcname string, r *PVCViewerReconciler, viewer *pvcviewerv1alpha1.PVCViewer) error {
	var nodename string
	...
	if err := r.List(context.Background(), pods, client.InNamespace(viewer.Namespace), client.MatchingFields{"spec.volumes.persistentvolumeclaim.claimname": pvcname}); err != nil {
	...
	//a nodeAffinity field so that the PVCViewer server will be scheduled (if possible)
