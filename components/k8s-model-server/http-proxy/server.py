...
define("rpc_timeout", default=1.0, help="seconds for time out rpc request",
       type=float)
define("rpc_port", default=9000, help="tf serving on the given port", type=int)
define("rpc_address", default='localhost',
       help="tf serving on the given address", type=str)
define("instances_key", default='instances',
...
define("request_log_prob", default=0.01,
...
def get_signature(signature_map, signature_name=None):
    ...
    # 1) if signature_name is specified, try to find it in the signature_map.
...
class MetadataHandler(tornado.web.RequestHandler):
    ...
    signature map with latest version).
    Defined here:
    ...
    def get(self, model_name):
        ...
        self.write(dict((key, MessageToDict(value)) for key, value in
                        signature_map.items()))


class PredictHandler(tornado.web.RequestHandler):
    """
    Predict Handler proxy predict method, the input of tf savedModel is expected
    to be a:
      `Map<strinbg, tf.Tensor>` protobuf. Defined here https://github.com/tensorflow/serving/blob/master/tensorflow_serving/apis/prediction_service.proto#L23  # noqa: E501
    """
    ...
    def post(self, model_name, version_name=None):
        ...
        signature_name_used, signature = get_signature(
            self.settings['signature_map'][model_name],
            signature_name)
        ...
        predictions = zip(
            *[tf.make_ndarray(result.outputs[output_key]).tolist() for
              output_key in output_keys])
        predictions = [dict(zip(*t)) for t in
                       zip(repeat(output_keys), predictions)]
...
class ClassifyHandler(tornado.web.RequestHandler):
    ...
    expected to be a `tf.Examples` protobuf
    Defined here:
