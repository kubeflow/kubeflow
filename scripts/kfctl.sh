...
createEnv() {
  ...
  case "$PLATFORM" in
    ...
    gcp)
      ...
      CLUSTER_VERSION=$(gcloud --project=${PROJECT} container get-server-config --zone=${ZONE} | awk '/validNodeVersions/{f=0} f; /validMasterVersions/{f=1}' | awk '{print $2}' | grep '1.10.' | head -1)
