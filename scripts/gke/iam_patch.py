...
# IAM policy patch can be defined in a separate file declaratively and it can either
# be added or removed from a projects iam policy
#
# Usage
# python iam_patch.py --action=add --project=agwliamtest --iam_bindings_file=iam_bindings.yaml
# python iam_patch.py --action=remove --project=agwliamtest --iam_bindings_file=iam_bindings.yaml
import argparse
import logging
import subprocess
import tempfile
...
def parse_args():
  ...
  parser.add_argument(
    "--action", default="add", type=str, help=("The action to take. Valid values: add, remove"))
  parser.add_argument(
  ...
  parser.add_argument('--dry_run', dest='dry_run', action='store_true', help=("Don't patch the final IAM policy, only print it"))
...
def get_current_iam_policy(project):
  ...
  """
  return yaml.load(subprocess.check_output(["gcloud", "projects", "get-iam-policy", project]))
...
def iam_policy_to_dict(bindings):
  ...
  for binding in bindings:
    ...
    for member in binding['members']:
      bindings_dict[role].add(member)
  return bindings_dict


def iam_dict_to_policy(bindings_dict):
  """
  iam_dict_to_policy takes an iam policy binding in the dict format and converts it into GCP API format
  so that it can be sent to GCP IAM API for an update
  """
  bindings = []
...
def patch_iam_policy(args):
  ...
  logging.info(yaml.dump(current_policy, default_flow_style=False, default_style=''))
  ...
  current_policy_bindings_dict = apply_iam_bindings_patch(current_policy_bindings_dict, bindings_patch, args.action)
  ...
  if not args.dry_run:
    subprocess.call(["gcloud", "projects", "set-iam-policy", args.project, updated_policy_file.name])
  else:
