...
def patch_iam_policy(args):
    ...
    current_policy_bindings_dict = iam_policy_to_dict(
    ...
    current_policy['bindings'] = iam_dict_to_policy(
        current_policy_bindings_dict)
    logging.info("Updated Policy")
    logging.info("\n" + yaml.dump(current_policy, default_flow_style=False,
                                  default_style=''))
    ...
    if not args.dry_run:
        subprocess.call(["gcloud", "projects", "set-iam-policy", args.project,
                         updated_policy_file.name])
    else:
        logging.info(
            "Skipping patching the IAM policy because --dry_run was set")
