        ...
        for root, _, files in os.walk(test_files_dir):
            for test_file in files:
                ...
                if should_test(full_path):
                    ...
                    try:
                        output = util.run(['flake8', full_path] + FLAKE8_OPTS,
                                          cwd=root)
                        try:
                        ...
                        if not hasattr(parsed, "get"):
                            ...
                            logging.warn(
                              ...
                              "Result was: %s. Output will be treated as a boolean",
                              output
                            )
                            test_passed = parsed
                        else:
                        ...
                        if not test_passed:
                            ...
                            logging.error(
                              '{}. See Subprocess output for details.'.
                              format(msg)
                            )
                        ...
                        test_case.add_failure_info(msg)
                        logging.error(
                          '{} with exception %s. See Subprocess output for '
