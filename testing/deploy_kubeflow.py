...
def deploy_kubeflow(test_case):
    ...
    if args.as_gcloud_user:
        ...
        # observe RBAC errors when doing certain operations. The problem
        # appears to be that we end up using the in cluster config (e.g. pod
        # service account) and not the GCP service account which has more
        # privileges.
