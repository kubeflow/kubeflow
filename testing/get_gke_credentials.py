...
def parse_args():
    ...
    parser.add_argument(
      ...
      type=str,
      help=(
        "The name of the cluster. If not set assumes the script is "
...
def get_gke_credentials(test_case):
    ...
    zone = args.zone
    logging.info(
      "Using cluster: %s in project: %s in zone: %s", cluster_name, project,
      zone
    )
    ...
    for user in config["users"]:
        ...
        if auth_provider.get("name") != "gcp":
            continue
        logging.info(
          "Modifying user %s which has gcp auth provider", user["name"]
        )
