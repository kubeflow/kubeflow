...
LOADTEST_SUCCESS = Gauge(
  'loadtest_success', 'number of successful requests in current load test'
)
SUCCESS_COUNT = Counter(
  'deployment_success_count', 'accumulative count of successful deployment'
)
FAILURE_COUNT = Counter(
  'deployment_failure_count', 'accumulative count of failed deployment'
)
LOADTEST_ZONE = [
...
def prepare_request_data(args, deployment):
    ...
    with open(
      os.path.join(FILE_PATH, "../bootstrap/config/gcp_prototype.yaml"), 'r'
    ) as conf_input:
    ...
    for param in defaultApp["parameters"]:
        ...
        if param["name"] == "hostname":
            param["value"
                 ] = "%s.endpoints.%s.cloud.goog" % (deployment, args.project)
...
def insert_ssl_cert(args, deployment):
    ...
    try:
        ...
        util_run((
          "gsutil cp gs://%s/* %s" %
          (get_gcs_path(args.mode, deployment), ssl_local_dir)
        ).split(' '))
    except Exception:
...
def create_secret(args, deployment, ssl_local_dir):
    util_run((
      ...
      (deployment, getZone(args, deployment), args.project)
    ).split(' '))
...
def check_deploy_status(args, deployments):
    ...
    while datetime.datetime.now() < end_time and len(deployments) > 0:
        ...
        for deployment in deployments:
            url = "https://%s.endpoints.%s.cloud.goog" % (
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logging.info(
                      "%s: IAP not ready, request number: %s" %
                      (deployment, num_req)
                    )
            except Exception:
                logging.info(
        ...
        for deployment in success_deploy:
            try:
                ...
                for sec in ["envoy-ingress-tls", "letsencrypt-prod-secret"]:
                    ...
                    with open(
                      os.path.join(ssl_local_dir, sec + ".yaml"), 'w+'
                    ) as sec_file:
                ...
                util_run((
                  "gsutil cp %s/* gs://%s/" %
                  (ssl_local_dir, get_gcs_path(args.mode, deployment))
                ).split(' '))
            except Exception:
...
def get_service_account_credentials(client_id_key):
    ...
    if isinstance(credentials, google.oauth2.credentials.Credentials):
        raise Exception(
          'make_iap_request is only supported for service '
          'accounts.'
        )
...
def clean_up_resource(args, deployments):
    ...
    while datetime.datetime.now() < end_time:
        ...
        try:
            ...
            response = request.execute()
            if ('deployments' not in response) or (
              len(
                deployments & set(d['name'] for d in response['deployments'])
              ) == 0
            ):
                delete_done = True
                break
...
def wrap_test(args):
    ...
    try:
        ...
        junit_path = os.path.join(
          args.artifacts_dir,
          "junit_kubeflow-deploy-{0}.xml".format(junit_name)
        )
...
def main(unparsed_args=None):
    ...
    util_run((
      ...
      may_get_env_var("GOOGLE_APPLICATION_CREDENTIALS")
    ).split(' '),
             cwd=FILE_PATH)
    if args.mode == "e2e":
        wrap_test(args)
