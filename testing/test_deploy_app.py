...
SERVICE_HEALTH = Gauge('deployment_service_status',
                       "0: normal; 1: deployment not successful; 2: service "
                       "down")
...
LOADTEST_ZONE = ['us-central1-a', 'us-central1-c', 'us-east1-c', 'us-east1-d',
                 'us-west1-b']
...
class requestThread(threading.Thread):
    ...
    def run(self):
        try:
            resp = requests.post("https://%s/kfctl/e2eDeploy" %
                                 self.target_url,
                                 json=self.req_data,
...
def prepare_request_data(args, deployment):
    ...
    with open(os.path.join(FILE_PATH,
                           "../bootstrap/config/gcp_prototype.yaml"), 'r') \
            as conf_input:
    ...
    for param in defaultApp["parameters"]:
        ...
        if param["name"] == "hostname":
            param["value"] = "%s.endpoints.%s.cloud.goog" % (
...
def make_e2e_call(args):
    ...
    if resp.status_code != 200:
        raise RuntimeError(
            "deploy request received status code: %s, message: %s" % (
...
def make_prober_call(args, service_account_credentials):
    ...
    try:
        resp = requests.post(
            "https://%s/kfctl/e2eDeploy" % get_target_url(args), json=req_data,
            headers={'Authorization': 'Bearer {}'.format(
...
# For each deployment, make a request to service url, return if all requests
# call successful.
def make_loadtest_call(args, service_account_credentials, deployments):
    ...
    for deployment in deployments:
        ...
        threads.append(requestThread(get_target_url(args), req_data,
                                     google_open_id_connect_token))
    for t in threads:
        t.start()
    for t in threads:
...
def insert_ssl_cert(args, deployment):
    ...
    while datetime.datetime.now() < end_time:
        sleep(5)
        try:
            request = service.deployments().get(project=args.project,
    ...
    try:
        ...
        util_run(("gsutil cp gs://%s/* %s" % (
            get_gcs_path(args.mode, deployment), ssl_local_dir)).split(' '))
    except Exception:
        ...
        for deployment in deployments:
            try:
                resp = requests.request(
                    METHOD, "https://%s.endpoints.%s.cloud.goog" % (
                        deployment, args.project),
                    headers={'Authorization': 'Bearer {}'.format(
                        google_open_id_connect_token)})
                if resp.status_code == 200:
                    ...
                    logging.info("%s: IAP not ready, request number: %s" % (
                        deployment, num_req))
            except Exception:
                logging.info(
                    "%s: IAP not ready, exception caught, request number: %s"
                    % (deployment, num_req))
        ...
        for deployment in success_deploy:
            try:
                ...
                if os.path.exists(ssl_local_dir):
                    continue
                os.makedirs(ssl_local_dir)
                util_run(("gcloud container clusters get-credentials "
                          "%s --zone %s --project %s" % (
                              deployment, getZone(args, deployment),
                              args.project)).split(' '))
                for sec in ["envoy-ingress-tls", "letsencrypt-prod-secret"]:
                    sec_data = util_run(
                        ("kubectl get secret %s -n kubeflow -o yaml"
                         % sec).split(' '))
...
# clean up deployment / app config from previous test deployments: set(string)
# which contains all deployment names in current test round.
def prober_clean_up_resource(args, deployments):
    logging.info("Clean up project resource (source repo, backend service and "
                 "deployment)")
    ...
    for zone in LOADTEST_ZONE:
        delete_gcloud_resource(args, 'instance-groups unmanaged',
    ...
    if args.mode == "prober":
        ...
        while True:
            ...
            if not prober_clean_up_resource(args, set([args.deployment])):
                ...
                FAILURE_COUNT.inc()
                logging.error(
                    "request cleanup failed, retry in %s seconds"
                    % args.wait_sec)
                continue
            PROBER_HEALTH.set(0)
            if make_prober_call(args, service_account_credentials):
                ...
                FAILURE_COUNT.inc()
                logging.error(
                    "prober request failed, retry in %s seconds"
                    % args.wait_sec)
