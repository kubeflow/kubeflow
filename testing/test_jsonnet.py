                ...
                if should_test(full_path):
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.error("Output of jsonnet eval could not be"
                                          "parsed as json; output: %s", output)
                            parsed = {}

                        if not hasattr(parsed, "get"):
                            # Legacy style tests emit true rather than a json
                            # object. Parsing the string as json converts it to
                            # a bool so we just use parsed as test_passed Old
                            # style tests actually use std.assert so jsonnet
                            # eval will actually return an error in the case
                            # the test didn't pass.
                            logging.warn("jsonnet test is using old style and "
                                         "not emitting an object. Result was: "
                                         "%s. Output will be treated as a "
                                         "boolean", output)
                            test_passed = parsed
                        else:
                            test_passed = parsed.get("pass", false)  # noqa: F821, E501

                        if not test_passed:
                            test_case.add_failure_info(
                                '{} test failed'.format(test_file))
                            logging.error('%s test failed. See Subprocess '
                                          'output for details.', test_file)
                    except Exception as e:
                        test_case.add_failure_info(
                            '{} test failed'.format(test_file))
                        logging.error('%s test failed with exception %s. '
                                      'See Subprocess output for details.', e,
                                      test_file)
