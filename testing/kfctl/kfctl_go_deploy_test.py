...
def get_kfctl_go_binary_path():
    """return the path to kfctl go binary.
...
def test_deploy_kfctl_go(app_path, project, use_basic_auth, use_istio, config_path):
    ...
    kfctl_path = get_kfctl_go_binary_path()
    ...
    run_with_retries([
        kfctl_path, "generate", "-V",
        "all", "--email=" + email, "--zone=" + zone], cwd=app_path)
