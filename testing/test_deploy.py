...
from kubeflow.testing import test_util, \
    util  # pylint: disable=no-name-in-module
...
def _setup_test(api_client, run_label):
    ...
    try:
        ...
        if e.status == 409:
            logging.info("Namespace %s already exists.",
...
def deploy_model(args):
    ...
    generate_command = ["ks", "generate", "tf-serving-deployment-gcp",
                        deployComponent]
    ...
    generate_command = ["ks", "generate", "tf-serving-service",
                        serviceComponent]
    util.run(generate_command, cwd=app_dir)
...
def deploy_argo(args):
    ...
    util.run(["kubectl", "create", "-n", "default", "-f",
    ...
    while True:
        ...
        output = util.run(["kubectl", "get", "pods", "-n", "default",
...
def deploy_minikube(args):
        ...
        "disks": [
            {
                ...
                "initializeParams": {
                    "sourceImage":
                        "projects/ubuntu-os-cloud/global/images/family/ubuntu-1604-lts",  # noqa: E501
                    "diskSizeGb":
                        100,
                    "autoDelete":
                        True,
    ...
    try:
        ...
        if content.get("error", {}).get("code") == requests.codes.CONFLICT:
            ...
            logging.error("Either the VM or the disk %s already exists in "
                          "zone %s in project %s ",
    ...
    # The .minikube directory contains some really large ISO and other files
    # that we don't need; so we only copy the files we need.
...
def main():  # pylint: disable=too-many-locals,too-many-statements
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "The GitHub API token to use. This is needed since ksonnet uses "
            "the GitHub API and without it we get rate limited. For more info "
            "see: https://github.com/ksonnet/ksonnet/blob/master/docs"
            "/troubleshooting.md. Can also be set using environment variable "
            "GITHUB_TOKEN."))

    parser.add_argument(
        "--deploy_name", default="", type=str,
    ...
    if not args.test_dir:
        ...
        now = datetime.datetime.now()
        label = "test_deploy-" + now.strftime("%m%d-%H%M-") + uuid.uuid4().hex[
            0:4]
    ...
    config_file = os.path.expanduser(kube_config.KUBE_CONFIG_DEFAULT_LOCATION)  # noqa: E501, F841
