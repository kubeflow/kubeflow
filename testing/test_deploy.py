...
def _setup_test(api_client, run_label):
    ...
    try:
        ...
        if e.status == 409:
            logging.info(
              "Namespace %s already exists.", namespace.metadata.name
            )
        else:
            raise
...
def setup_kubeflow_ks_app(args, api_client):
    ...
    source = os.path.join(
      args.test_dir, "src", REPO_ORG, REPO_NAME, REGISTRY_PATH
    )
...
def deploy_pytorchjob(args):
    ...
    ks_deploy(
      app_dir, component, params, env=None, account=None, namespace=None
    )
...
def wrap_test(args):
    ...
    try:
        ...
        junit_path = os.path.join(
          args.artifacts_dir, "junit_{0}.xml".format(junit_name)
        )
    ...
    for cluster in config["clusters"]:
        ...
        for user in config["users"]:
            for k in ["client-certificate", "client-key"]:
                user["user"][k] = os.path.join(
                  certs_dir, os.path.basename(user["user"][k])
                )
...
def deploy_minikube(args):
    ...
    if final_op.get("status") != "DONE":
        raise ValueError(
          "Insert operation has status %s", final_op.get("status")
        )
    ...
    if not os.path.exists(install_script):
        logging.error(
          "Could not find minikube install script: %s", install_script
        )

    vm_util.wait_for_vm(args.project, args.zone, args.vm_name)
    vm_util.execute_script(
      args.project, args.zone, args.vm_name, install_script
    )
...
def teardown_minikube(args):
    ...
    if final_op.get("status") != "DONE":
        raise ValueError(
          "Delete operation has status %s", final_op.get("status")
        )
    ...
    try:
        ...
        if content.get("error", {}).get("code") == requests.codes.NOT_FOUND:
            logging.info(
              "Disk %s in zone %s in project %s already deleted.", args.vm_name,
              args.zone, args.project
            )
        else:
            raise
    ...
    if response:
        ...
        op_id = response.get("name")
        final_op = vm_util.wait_for_operation(
          gce, args.project, args.zone, op_id
        )

        logging.info("Final result for disk delete operation: %s", final_op)
        if final_op.get("status") != "DONE":
            raise ValueError(
              "Disk delete operation has status %s", final_op.get("status")
            )
...
def main():  # pylint: disable=too-many-locals,too-many-statements
    ...
    parser.add_argument(
      ...
      action="store_true",
      help=(
        "Impersonate the user corresponding to the gcloud "
