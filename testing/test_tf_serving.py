...
def almost_equal(a, b, tol=0.001):
    """
    Compares two json objects (assuming same structure) with tolerance on
    numbers
    """
    if isinstance(a, dict):
        ...
        for i in xrange(len(a)):  # noqa: F821
...
def main():
    ...
    try:
        ...
        model_urls = [
            "http://" + service_ip + ":8500/v1/models/mnist:predict",
            # tf serving's http server
        ]
        for model_url in model_urls:
            ...
            if args.result_path:
                with open(args.result_path) as f:
                    ...
                    assert (
                        almost_equal(expected_result, json.loads(result.text)))
    except Exception as e:
