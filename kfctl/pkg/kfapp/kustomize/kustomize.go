...
import (
	...
	kfapisv2 "github.com/kubeflow/kubeflow/kfctl/v2/pkg/apis"
...
// TODO based on kfctl/app/k8sUtil.go. Need to merge.
...
func GenerateKustomizationFile(kfDef *kfdefsv2.KfDef, root string,
	...
	factory := resmap.NewFactory(resource.NewFactory(uf))
	pluginConfig := plugins.DefaultPluginConfig()
	pl := plugins.NewLoader(pluginConfig, factory)
	...
	_loader, loaderErr := loader.NewLoader(loader.RestrictionRootOnly, v, compDir, fsys)
	if loaderErr != nil {
		return nil, fmt.Errorf("could not load kustomize loader: %v", loaderErr)
	}

	defer _loader.Cleanup()
	kt, err := target.NewKustTarget(_loader, factory, transformer.NewFactoryImpl(), pl)
	if err != nil {
		return nil, err
	}
