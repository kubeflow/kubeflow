local params = std.extVar("__ksonnet/params").components["core7"];
...
// local diskNames = if diskparm != "null" && std.length(diskParam) > 0 then
local diskNames = if diskparm != "null" then
  std.split(diskParam, ',')
  else [];
